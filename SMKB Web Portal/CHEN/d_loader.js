...
function load_loaderPercent(msg) {
    if (msg === undefined || msg === null) {
        msg = "";
    }
    ...
    lbl.innerHTML = msg;
    lbls.innerHTML = "Sila tunggu hingga proses selesai..."
...
function upd_loadertext(newMsg) {
    ...
    //pgloaders.style.cssText = "width:50%;z-index:1003;font-size:10px;color:black;position:relative;margin-top:10px;margin-left:0px;"
...
function show_loaderPercent(msg) {
    _x0prntobj = document;
    loadLoaderPercent(msg);
}
function show_loaderP(msg) {
  _x0prntobj = window.parent.document;
  loadLoader(msg);
}
function loadLoaderPercent(msg) {
    //GoTop();
    ...
    if (_x0prntobj.getElementById("sessionContainer") === undefined || _x0prntobj.getElementById("sessionContainer") === null) {
        ...
        _x0prntobj.body.appendChild(load_loaderPercent(msg));
    } else {
        upd_loadertext(msg);
    }
}
