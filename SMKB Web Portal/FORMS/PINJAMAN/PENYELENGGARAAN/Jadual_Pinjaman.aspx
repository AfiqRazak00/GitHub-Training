<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/NestedMasterPage2.master" CodeBehind="Jadual_Pinjaman.aspx.vb" Inherits="SMKB_Web_Portal.Jadual_Pinjaman" %>
<asp:Content ID="Content1" ContentPlaceHolderID="FormContents" runat="server">
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <div class="modal-body">
                <div class="table-title">
                    <h6>Senarai Jadual Pembiayaan</h6>
                    <div id="btnTambah" class="btn btn-primary">
                        <i class="fa fa-plus"></i>Jadual Pembiayaan 
                    </div>
                </div>
            </div>
            <br />
        ...
        <!-- Modal Add/Edit Jadual Pinjaman -->
        <div class="modal fade" id="mdlJdlPinjaman" tabindex="-1" role="dialog"
            aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title"">Maklumat Jadual Pembiayaan</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="form-row col-md-12 col-12">
                                        <div class="form-group input-group col-md-6 col-12 ">
                                            <input type="text" class="tempoh input-group__input" placeholder="" value="" />
                                            <label class="input-group__label">Tempoh (Bulan) <span style="color: red">*</span></label>
                                            <small class="txtEror text-danger font-italic form-row col-md-12 col-12">Sila isi ruangan ini</small>
                                        </div>
                                        <div class="form-group input-group col-md-6 col-12">
                                            <input type="text" class="amaun input-group__input" placeholder="" value="" />
                                            <label class="input-group__label">Amaun (RM) <span style="color: red">*</span></label>
                                            <small class="txtEror text-danger font-italic form-row col-md-12 col-12">Sila isi ruangan ini</small>
                                        </div>
                                    </div>
                                    <div class="col-md-12 col-12"></div>
                                    <div class="form-row col-md-12 col-12">
                                        <div class="form-group input-group col-md-6 col-12">
                                            <input type="text" class="ansuran input-group__input" placeholder="" value="" />
                                            <label class="input-group__label">Ansuran (RM) <span style="color:red">*</span></label>
                                            <small class="txtEror text-danger font-italic form-row col-md-12 col-12">Sila isi ruangan ini</small>
                                        </div>
                                    </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            generateDropdown("ddlKatPinj", "JadualPinjWS.asmx/GetKategoriPinjaman", "Pilih Kategori Pembiayaan", false, null);
        });
        ...
        var dttable = $("#tblSenarai").DataTable({
            ...
            "rowCallback": async function (row, data, index) {
                ...
                $(row).off('click').on('click', async function () {
                    ...
                    targetId = value;
                    var rdata = await ajaxPost("JadualPinjWS.asmx/GetSelectedRow", { id: value }, false);
        ...
        $("#btnSimpan").off('click').on('click', async function () {
            ...
            var amaun = isSet($(".amaun").val().replace(/,/g, ''));
            var ansuran = isSet($(".ansuran").val().replace(/,/g, ''));
            var tempoh = isSet($(".tempoh").val());

            if (isEdit) {
                if (slctdStatus && amaun && ansuran && tempoh) {

                    $("#mdlJdlPinjaman").modal('hide');

                    var postDt = {
                        Amaun: viewDt($(".amaun").val().replace(/,/g, ''), 'double'),
                        Ansuran: viewDt($(".ansuran").val().replace(/,/g, ''), 'double'),
                        Tempoh: viewDt($(".tempoh").val(), 'text'),
                        Status: viewDt($('input[name="statusRadio"]:checked').val(), 'int'),
                        TargetId: viewDt(targetId.toString(), 'text'),
                    };

                    var data = await ajaxPost("JadualPinjWS.asmx/KemaskiniJdlPinjaman", postDt, true);
                    ...
                    if (data.Status) {
                        ...
                        dttable.cell(adjustedRowIndex, 2).data(postDt.Tempoh); // Gaji Ke
                        dttable.cell(adjustedRowIndex, 3).data($(".amaun").val()); // Max Tempoh
                        dttable.cell(adjustedRowIndex, 4).data($(".ansuran").val()); // Max Amaun
                    ...
                    const elementsToCheck = [
                        { condition: !amaun, selector: ".amaun" },
                        { condition: !ansuran, selector: ".ansuran" },
                        { condition: !tempoh, selector: ".tempoh" },
                    ];
                ...
                if (ddlKatPinj && slctdStatus && amaun && ansuran && tempoh) {
                    ...
                    var data = await ajaxPost("JadualPinjWS.asmx/TambahJadualPinjaman", postDt, true);
        ...
        function clearAndEnableEdit() {
            ...
            $('#mdlJdlPinjaman .input-group__select select').each(function () {
            ...
            $('#mdlJdlPinjaman .input-group__input').each(function () {
                const element = $(this).closest('.form-group').find('.txtEror');
        ...
        async function loadSenarai() {
            ...
            var data = await ajaxPost("JadualPinjWS.asmx/FetchSenarai", {}, true);
