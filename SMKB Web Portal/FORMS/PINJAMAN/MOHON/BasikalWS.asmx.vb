...
Public Class BasikalWS
    ...
    <System.Web.Services.WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function GetJenamaKenderaan(ByVal q As String) As String
        Dim tmpDT As DataTable = GetDataJenamaKenderaan(q)
        Return JsonConvert.SerializeObject(tmpDT)
    End Function

    Private Function GetDataJenamaKenderaan(q As String) As DataTable
        Dim dtbl2 As DataTable
        Dim db As New DBKewConn
        ...
        Dim query As String = "SELECT Kod_Detail as value, Butiran as text FROM SMKB_Lookup_Detail WHERE Kod = 'PJM05' and STATUS = '1' 
                               and Kod_Detail like 'js%'"
        Dim param As New List(Of SqlParameter)
    ...
    Function GetSenaraiSemak(postData As String) As String
        ...
        Dim query As String = $"Select ID_CheckList, (SELECT Butiran FROM SMKB_Lookup_Detail WHERE Kod = 'PJM29' AND Kod_Detail = ID_Butiran) AS Butiran
                                ...
                                Where Kategori_Pinj = 'K00001' And Jenis_Pinj = 'J00005' And Taraf_Khidmat = '{postDt("KodTarafK")}' And Status = '1'"

        'Dim query As String = $"Select * From SMKB_Pinjaman_Check_List
        '                        Where Kat_Pinj = 'K00001' And Jenis_Pinj = 'J00005' And Status = '1'"
    ...
    Private Function GetTempohKelayakan(postDt) As Dictionary(Of String, Object)
        ...
        Dim query As String = $"Select Status_Dok, No_Pinj, No_Staf, tkh_mohon, DATEADD(Year, 3, tkh_mohon) AS tigatahuntempoh From SMKB_Pinjaman_Hdr
                              Where No_Staf = '{Session("ssusrID")}' AND Kategori_Pinj = 'K00001'
                              And tkh_mohon = (Select Max (tkh_mohon) From SMKB_Pinjaman_Hdr Where No_Staf = '{Session("ssusrID")}' And Kategori_Pinj = 'K00001' AND Status_Dok <> '28')"
    ...
    Private Function InsertMaklumatKend(data, noPinjaman) As String
        ...
        Dim query As String = "INSERT INTO SMKB_Pinjaman_Dtl_Kenderaan 
                                (No_Pinj, No_Casis, Kod_Buatan, Kod_Model, Kod_Kelas_Kenderaan, Thn_Buat, 
                                Harga_Bersih, Status, No_Pendaftaran) 
                                VALUES (@a, @b, @c, @d, @e, @f, @g, @h, @i)"

        Dim param As New List(Of SqlParameter)

        param.Add(New SqlParameter("@a", noPinjaman))
        param.Add(New SqlParameter("@b", data("NoChasis")))
        param.Add(New SqlParameter("@c", data("JenamaKend")))
        param.Add(New SqlParameter("@d", data("MdlKend")))
        param.Add(New SqlParameter("@e", data("KelasKend")))
        param.Add(New SqlParameter("@f", data("TahunBuat")))
        param.Add(New SqlParameter("@g", data("HargaBersih")))
        param.Add(New SqlParameter("@h", "1"))
        param.Add(New SqlParameter("@i", data("NoPendaftaran")))
    ...
    Public Function KemaskiniPermohonanKenderaan(postData As String) As String
        ...
        Dim query2 As String = $"Update SMKB_Pinjaman_Dtl_Kenderaan Set
                            Kod_Buatan= '{postDt("JenamaKend")}', No_Casis= '{postDt("NoChasis")}', Kod_Model= '{postDt("MdlKend")}', 
                            Kod_Kelas_Kenderaan= '{postDt("KelasKend")}', Thn_Buat= '{postDt("TahunBuat")}', 
                            Harga_Bersih= '{postDt("HargaBersih")}',
                            No_Pendaftaran= '{postDt("NoPendaftaran")}'
                            Where No_Pinj = '{NoPinjaman}'"
    ...
    Public Function PermohonanLengkap(postData As String) As String
        ...
        'Check Jika Data Ada
        Dim query0 As String = $"Select * From SMKB_Pinjaman_Hdr Where No_Staf = '{Session("ssusrID")}' And Kategori_Pinj = 'K00001' And Status_Dok = '01'"
    ...
    Private Function ProsesEmelKePenjamin(sendDt) As String
        ...
        If dtbl.Rows.Count > 0 Then
            Dim subject As String = $"PENGUJIAN PERMOHONAN SKIM PEMBIAYAAN BASIKAL STAF UTeM"
            Dim body As String = "<html><body>" &
                               "<p style='font-size:16px; font-weight:bold;'>PENGUJIAN PERMOHONAN SKIM PEMBIAYAAN BASIKAL STAF UTeM</p><br/><br/>" &
                               $"<p>Dimaklumkan bahawa, tuan/puan telah disenaraikan sebagai penjamin bagi permohonan basikal {dtbl2.Rows(0).Item("NamaPeminjam")}</p>" &
