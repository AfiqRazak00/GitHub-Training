...
Public Class DashboardAR_WS
    ...
    Private Function GetJumTerimaByYear() As DataTable
        Dim db As New DBKewConn
        Dim query As String = "SELECT 
                                    CASE WHEN SUM(A.Jumlah) IS NULL THEN '0.00' ELSE CAST(SUM(A.Jumlah) AS DECIMAL(10, 2)) END AS JUMLAH_AKAUN_BELUM_TERIMA,
                                    B.Tahun
                                FROM 
                                    SMKB_Bil_Hdr A
                                INNER JOIN 
                                    SMKB_Bil_Dtl B ON A.No_Bil = B.No_Bil 
                                WHERE 
                                    Kod_Status_Dok = @kod
                                GROUP BY 
                                    B.Tahun 
    ...
    Private Function GetJumTerima() As DataTable
        Dim db As New DBKewConn
        Dim query As String = "SELECT 
                                    CASE WHEN SUM(A.Jumlah) IS NULL THEN '0.00' 
	                                ELSE CAST(SUM(A.Jumlah) AS DECIMAL(10, 2)) END AS JUMLAH_AKAUN_BELUM_TERIMA
                                FROM 
                                    SMKB_Bil_Hdr A
                                INNER JOIN 
                                    SMKB_Bil_Dtl B ON A.No_Bil = B.No_Bil 
    ...
    Private Function GetJumTunggak() As DataTable
        ...
        Dim query As String = "SELECT 
                                    CASE 
                                        WHEN Total_Tunggak IS NULL THEN '0.00'
                                        ELSE Total_Tunggak
                                    END AS Total_Tunggak
                                FROM
                                    (
                                    SELECT 
                                        (
                                            (SELECT 
                                                CASE WHEN SUM(A.Jumlah) IS NULL THEN '0.00' ELSE SUM(A.Jumlah) END AS JUMLAH 
                                            FROM 
                                                SMKB_Bil_Hdr A
                                            INNER JOIN 
                                                SMKB_Bil_Dtl B ON A.No_Bil = B.No_Bil 
                                            WHERE 
                                                Kontrak = '1' AND Tkh_Tamat < GETDATE() AND Kod_Status_Dok = '03') + 
                                            (
                                                SELECT 
                                                    SUM(A.Jumlah) - 
                                                    (SELECT 
                                                    ...
                                                    INNER JOIN 
                                                        SMKB_Bil_Dtl B ON A.No_Bil = B.No_Bil 
                                                    INNER JOIN 
                                                        SMKB_Terima_Hdr C ON A.No_Bil = C.No_Rujukan
                                                    WHERE 
                                                        A.Kontrak = '0' AND C.Kod_Status_Dok = '06'
                                                    ) AS JUMLAH 
                                                FROM 
                                                    SMKB_Bil_Hdr A
                                                INNER JOIN 
                                                    SMKB_Bil_Dtl B ON A.No_Bil = B.No_Bil 
                                                WHERE 
                                                    Kontrak = '0' AND Kod_Status_Dok = '03'
                                            )
                                        ) AS Total_Tunggak
                                    ) AS SubqueryAlias;
    ...
    Private Function GetJumTunggakPeratus(totalBill As String) As DataTable
        ...
        Dim query As String = "SELECT 
                                CASE 
                                    WHEN (((SELECT SUM(A.Jumlah) FROM SMKB_Bil_Hdr A
                                           INNER JOIN SMKB_Bil_Dtl B ON A.No_Bil=B.No_Bil 
                                           WHERE Kontrak='1' AND Tkh_Tamat<GETDATE() AND Kod_Status_Dok='03') +
                                          (SELECT SUM(A.Jumlah) - 
                                                  (SELECT SUM(C.Jumlah_Bayar) 
                                                   FROM SMKB_Bil_Hdr A
                                                   INNER JOIN SMKB_Bil_Dtl B ON A.No_Bil=B.No_Bil 
                                           ...
                                           FROM SMKB_Bil_Hdr A
                                           INNER JOIN SMKB_Bil_Dtl B ON A.No_Bil=B.No_Bil 
                                           WHERE Kontrak='0' AND Kod_Status_Dok='03')) * 100) IS NULL THEN 0
                                    ELSE
                                        (((SELECT SUM(A.Jumlah) FROM SMKB_Bil_Hdr A
                                           INNER JOIN SMKB_Bil_Dtl B ON A.No_Bil=B.No_Bil 
                                           WHERE Kontrak='1' AND Tkh_Tamat<GETDATE() AND Kod_Status_Dok='03') +
                                          (SELECT SUM(A.Jumlah) - 
                                                  (SELECT SUM(C.Jumlah_Bayar) 
                                           ...
                                           FROM SMKB_Bil_Hdr A
                                           INNER JOIN SMKB_Bil_Dtl B ON A.No_Bil=B.No_Bil 
                                           WHERE Kontrak='0' AND Kod_Status_Dok='03')) * 100 / @totalBill)
                                END AS PeratusTunggakan;
"
