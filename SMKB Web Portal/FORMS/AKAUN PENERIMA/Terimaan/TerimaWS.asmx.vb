...
Public Class TerimaWS
    ...
    Private Function GetOrder_SenaraiLulusTerima(category_filter As String, tkhMula As DateTime, tkhTamat As DateTime) As DataTable
        ...
        Dim query As String = "SELECT A.No_Dok ,B.Nama_Penghutang,C.Butiran as UrusNiaga,A.Tujuan,
                                ...
                                LEFT JOIN VPeribadi as F on F.MS01_NoStaf = A.Staf_Terima
    ...
    Private Function UpdateLulusOrder(orderid As Order_terima)
        ...
        Dim query As String = "UPDATE SMKB_Terima_Hdr
                                SET Flag_Lulus=@flaglulus,Tkh_Lulus=@tkhlulus,Staf_Lulus=@staflulus,Kod_Status_Dok=@Kod_status_dok
                                WHERE No_Dok=@id"
        Dim param As New List(Of SqlParameter)
    ...
    End Function
    Private Function UpdateXLulusOrder(orderid As Order_terima)
        Dim db As New DBKewConn

        Dim kodstatusLulus As String = "09" 'Kelulusan Resit
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function RejectLulus(order As Order_terima) As String
        Dim resp As New ResponseRepository
        resp.Success("Transaksi Berjaya")
        Dim success As Integer = 0
        Dim JumRekod As Integer = 0
        If order Is Nothing Then
            resp.Failed("Tiada Rekod disimpan")
            Return JsonConvert.SerializeObject(resp.GetResult())
        End If



        If UpdateXLulusOrder(order) <> "OK" Then
