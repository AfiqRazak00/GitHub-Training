<%@ Page Language="vb" AutoEventWireup="false" MasterPageFile="~/NestedMasterPage2.master" CodeBehind="Pendahuluan_AR.aspx.vb" Inherits="SMKB_Web_Portal.Pendahuluan_AR" %>
             ...
             <div>
                 <div class="modal-body">
                     <div class="modal-header">
                         <h5 class="modal-title">Pendahuluan</h5>
                     </div>
                 ...
                 <div class="modal-dialog-scrollable">
                     <div class="modal-content">
                         <div class="modal-header modal-header--sticky">
                             <h5 class="modal-title" id="ModalCenterTitle">Pendahuluan Bayaran</h5>
 ...
 <script type="text/javascript">
     function ShowDetails(id, status) {
         if (id !== "") {
             if (status !== "") {
                 if (status == "NOTDONE") {
                     location.replace('<%=ResolveClientUrl("~/FORMS/AKAUN PENERIMA/Integrasi/PENDAHULUAN_TERPERINCI.aspx?kodsubmenu=btnMenu121804")%>&kodpenghutang=' + id + '&status=03', '_blank');
            } else if (status == "DONE") {
                location.replace('<%=ResolveClientUrl("~/FORMS/AKAUN PENERIMA/Integrasi/PENDAHULUAN_TERPERINCI.aspx?kodsubmenu=btnMenu121804")%>&kodpenghutang=' + id + '&status=04', '_blank');
                 }
             }
         }
     }
     ...
     $(document).ready(function () {
         ...
         tbl = $("#tblDataSenarai").DataTable({
             ...
             "ajax": {
                 "url": "IntegrasiWS.asmx/LoadOrderRecord_SenaraiLulusPendahuluan",
                 "method": 'POST',
         ...
         $('.btnSearch').click(async function () {
             ...
             <%--if (categoryFilter == "6") {
                 ...
                 const url = `<%=ResolveClientUrl("~/FORMS/AKAUN PENERIMA/INVOIS/PENDAHULUAN_AR.aspx?kodsubmenu=btnMenu121804")%>&categoryFilter=${categoryFilter}&startDate=${startDate}&endDate=${endDate}`;

                 localStorage.setItem('redirectTerimaanUrl', url);
                 location.replace(url, '_blank');
             } else {

                 const url = `<%=ResolveClientUrl("~/FORMS/AKAUN PENERIMA/INVOIS/PENDAHULUAN_AR.aspx?kodsubmenu=btnMenu121804")%>&categoryFilter=${categoryFilter}`;
                 localStorage.setItem('redirectTerimaanUrl', url);
                 location.replace(url, '_blank');
             }--%>
