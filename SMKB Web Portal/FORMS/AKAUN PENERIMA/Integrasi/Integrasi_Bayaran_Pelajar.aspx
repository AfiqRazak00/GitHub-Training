...
<asp:Content ID="Content1" ContentPlaceHolderID="FormContents" runat="server">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="search-filter">
                            <div class="form-row form-group row col-sm-12">
                                <div class="col-sm-3"></div>
                                    ...
                                    <div class="col-sm-3">
                                    <select id="ddlKategoriPelajar" class="ui search dropdown custom-select" name="ddlKategoriPelajar"></select>
                                </div>
                            </div>
                            <br />
                                ...
                                <br />
                                 <div class="form-row form-group row col-sm-12">
                                    ...
                                    <div class="col-sm-3">
                                     ...
                                     <button id="btnSearchPenaja" runat="server" class="btn btnSearchPenaja" 
                                        type="button">
                            ...
                            <div id="search_individu" class="justify-content-center search_individu">
                                ...
                                <div class="form-row form-group row col-sm-12">
                                    ...
                                    <button id="btnSearch" runat="server" class="btn btnSearch"
                                        type="button">
                                        <i class="fa fa-search"></i>
                                    </button>
                                </div>
                                        ...
                                        <table id="tblDataSenaraiPenaja" class="table table-striped " style="width: 100%!important">
                                            <thead >
                                                <tr style="width: 100%!important">
                                                    <th style="width:5%;display:none" ><input type="checkbox" id="selectAll"  ><label for="selectAll"></label></th>
                                                    <th scope="col" style="width: 15%">No. Akaun</th>
                                                    <th scope="col" style="width: 25%">Nama</th>
                                                    <th scope="col" style="width: 10%">No. Rujukan</th>
                        ...
                        <div class="modal-footer modal-footer--sticky" id="footerpenaja" style="display:none">
                            <div class="form-group col-md-12" align="right">
                                ...
                                <button type="button" class="btn btn-success btnBayar" data-toggle="tooltip" data-placement="bottom" title="Buat Bayaran" onclick="bayar_penaja()">Bayar</button>
    ...
    <script type="text/javascript">
        ...
        function ShowDetails(id, status) {
            if (id !== "") {
                if (status !== "") {
                    var KatPelajar = $('#ddlKategoriPelajar').val()
                    var Sesi = $('#ddlSesi').val()
                    var Fakulti = $('#ddlFakulti').val()
                    if (status == "NOTDONE") {
                        location.replace('<%=ResolveClientUrl("~/FORMS/AKAUN PENERIMA/Integrasi/Integrasi_Pelajar_Terperinci.aspx")%>?KodPenghutang=' + id + '&status=03' + '&kat=IND' + '&KatPelajar=' + KatPelajar + '&Sesi=' + Sesi + '&Fakulti=' + Fakulti, '_blank');
                    } else if (status == "DONE") {
                        location.replace('<%=ResolveClientUrl("~/FORMS/AKAUN PENERIMA/Terimaan/Integrasi_Pelajar_Terperinci.aspx")%>?KodPenghutang=' + id + '&status=04' + '&kat=IND', '_blank');
        ...
        $(document).ready(function () {
            ...
            tbl = $("#tblDataSenarai").DataTable({
                ...
                "ajax": {
                    ...
                    "data": function () {
                        ...
                        //var Fakulti = $('#ddlFakulti').val()
                        const searchParams = new URLSearchParams(window.location.search);
                        var KatPelajar = searchParams.get('KatPel')
                        var Sesi = searchParams.get('Sesi')
                        var Fakulti = searchParams.get('Fakulti')

                        KatPelajar = KatPelajar == null ? "" : KatPelajar;
                        Sesi = Sesi == null ? "" : Sesi;
                        Fakulti = Fakulti == null ? "" : Fakulti;
                        return JSON.stringify({
                            Fakulti: Fakulti,
                            isClicked: isClicked,
                            KatPelajar: KatPelajar,
                            Sesi: Sesi
                        })
                        //akhir sini
                    }
            ...
            $('.btnSearch').click(async function () {
                var Fakulti = $('#ddlFakulti').val();
                var KatPel = $('#ddlKategoriPelajar').val()
                var Sesi = $('#ddlSesi').val()
                var kategori = 'IND'
                var Nama_Fakulti = $('#ddlFakulti').dropdown('get text');
                Nama_Fakulti = Nama_Fakulti.replace('&',' AOA ')
                const url = `<%=ResolveClientUrl("~/FORMS/AKAUN PENERIMA/Integrasi/Integrasi_Bayaran_Pelajar.aspx?kodsubmenu=btnMenu121801")%>&Fakulti=${Fakulti}&Nama_Fakulti=${Nama_Fakulti}&KatPel=${KatPel}&Sesi=${Sesi}&kategori=${kategori}`;

                    localStorage.setItem('redirectIntegrasiPelajarUrl', url);
                    location.replace(url, '_blank');
            })
            const searchParams_1 = new URLSearchParams(window.location.search);
            if (searchParams_1.has('kategori')) {
                loadTable();
                
                var KatPel = searchParams_1.get('KatPel')
                var Butir_Kategori
                var Sesi = searchParams_1.get('Sesi')
                var Fakulti = searchParams_1.get('Fakulti')
                var Butir_Fakulti
                var kategori = searchParams_1.get('kategori')
                var penaja = searchParams_1.get('Penaja')
                if (kategori == "PN") {
                    // append ddlSesi data
                    $('#ddlSesiPenaja').dropdown('set selected', Sesi);
                    $('#ddlSesiPenaja').append("<option value = '" + Sesi + "'>" + Sesi + "</option>")
                    // append ddlSesi data
                    $('#ddlKategoriPenghutang').dropdown('set selected', penaja);
                    $('#ddlKategoriPenghutang').append("<option value = '" + penaja + "'>" + penaja + "</option>")
                } else if (kategori == "IND") {
                    // append ddlSesi data
                    $('#ddlSesi').dropdown('set selected', Sesi);
                    $('#ddlSesi').append("<option value = '" + Sesi + "'>" + Sesi + "</option>")
                }

                if (KatPel == "PG") {
                    Butir_Kategori = "PELAJAR PASCA SISWAZAH";
                } else if (KatPel == "PL") {
                    Butir_Kategori = "PELAJAR SARJANA MUDA";
                } else if (KatPel == "PH") {
                    Butir_Kategori = "PELAJAR SEPANJANG HAYAT";
                }
                if (searchParams_1.has('Nama_Fakulti')) {
                    Butir_Fakulti = searchParams_1.get('Nama_Fakulti');
                    Butir_Fakulti = Butir_Fakulti.replace(' AOA ', '&')
                }

                // append ddlKategoriPelajar data
                $('#ddlKategoriPelajar').dropdown('set selected', KatPel);
                $('#ddlKategoriPelajar').append("<option value = '" + KatPel + "'>" + Butir_Kategori + "</option>")
                
                // append ddlFakulti data
                $('#ddlFakulti').dropdown('set selected', Fakulti);
                $('#ddlFakulti').append("<option value = '" + Fakulti + "'>" + Butir_Fakulti + "</option>")
            ...
            tbl_Penaja = $("#tblDataSenaraiPenaja").DataTable({
                ...
                "ajax": {
                    ...
                    "data": function () {
                        ...
                        var KatPelajar = searchParams.get('KatPel')
                        var Sesi = searchParams.get('Sesi')
                        var Penaja = searchParams.get('Penaja')

                        KatPelajar = KatPelajar == null ? "" : KatPelajar;
                        Sesi = Sesi == null ? "" : Sesi;
                        Penaja = Penaja == null ? "" : Penaja;
                        return JSON.stringify({
                            Penaja: Penaja,
                            isClicked: isClicked,
                            KatPelajar: KatPelajar,
                        ...
                        return JSON.stringify({
                            Penaja: $('#ddlKategoriPenghutang').val(),
                            isClicked: isClicked,
                            KatPelajar: $('#ddlKategoriPelajar').val(),
                            Sesi: $('#ddlSesiPenaja').val()
                        })
                        //akhir sini
                    }
            ...
            $('.btnSearchPenaja').click(async function () {
                ...
                var Penaja = $('#ddlKategoriPenghutang').val();
                var Nama_Penaja = $('#ddlKategoriPenghutang').dropdown('get text');
                var kategori = 'PN'
                const url = `<%=ResolveClientUrl("~/FORMS/AKAUN PENERIMA/Integrasi/Integrasi_Bayaran_Pelajar.aspx?kodsubmenu=btnMenu121801")%>&Penaja=${Penaja}&KatPel=${KatPel}&Sesi=${Sesi}&Nama_Penaja=${Nama_Penaja}&kategori=${kategori}`;
            ...
            ////alert("searchParams" + searchParams )
            ////if (Fakulti !== null) {
            ...
            //}
            //if (Fakulti !== null) {
            //    var value = searchParams.get('categoryFilter');
            //    getFakulti(value)
            //        .then(data => {
            //            const parsedData = JSON.parse(data);
            //            // Assuming parsedData is an array of objects with 'value' and 'text' properties
            //            // Adjust this according to the structure of your data
            //            for (const entry of parsedData) {
            //                var option = $('<option>').attr('value', entry.value).text(entry.text);
            //                $('#ddlFakulti').append(option);
            //            }
            //        })
            //        .catch(error => {
        ...
        function loadJumlah() {
            let gJumlahRaw = 0; // Initialize the sum variable
            let data = tbl_Penaja.rows().data()
            data.each(function (rowData) {
                let jumlah_raw = readInt(rowData.Jumlah_Keseluruhan);
                gJumlahRaw += jumlah_raw;
            });
            $("#stickyJumlah").html(formatPrice(gJumlahRaw));
            $("#stickyJumlahItem").html(data.length);
        }
