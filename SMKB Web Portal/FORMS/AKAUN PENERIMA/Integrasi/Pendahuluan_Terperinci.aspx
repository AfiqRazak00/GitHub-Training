<%@ Page Title="" Language="vb" Async="true" AutoEventWireup="false" MasterPageFile="~/NestedMasterPage2.master" CodeBehind="Pendahuluan_Terperinci.aspx.vb" Inherits="SMKB_Web_Portal.Pendahuluan_Terperinci" %>
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="transaction-table table-responsive">
                                    <table id="tblDataSenarai" class="table table-striped">
                                        <thead>
                                            <tr style="width: 100%">
                                                <th scope="col" style="width: 10%" id="lblColumnBil">No. Pendahuluan</th>
            ...
            <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
                ...
                <div class="modal-content">
                    <div class="modal-header modal-header--sticky">
                        ...
                        <label id="txtNoResit" style="visibility: hidden">No Resit</label>
                            ...
                            <div class="col-md-12">
                                <div class="form-row">
                                    <div class="form-group col-md-3">
                                        ...
                                        <label class="input-group__label" for="txtnoinv" id="lblModalBil">No. Pendahuluan</label>
                                    </div>
                                    <div class="form-group col-md-3">
                                        <input type="text" class="form-control input-group__input" id="txtNamaPenghutang" name="txtNamaPenghutang" readonly />
                                       <label class="input-group__label" for="ddlPenghutang">Penghutang</label>
                                         <div style="display: none">
                                                <select id="ddlPenghutang" class="ui search dropdown" name="ddlPenghutang"></select>
                                            </div>
                                    </div>
                                    <div class="form-group col-md-3">
                                        <input type="text" class="form-control input-group__input" id="tkhMula" name="tkhMula" readonly />
                                        <label class="input-group__label" for="tkhMula">Tarikh Mohon</label>
                                    </div>
                                    <div class="form-group col-md-3">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            tbl = $("#tblDataSenarai").DataTable({
                ...
                "ajax": {
                    "url": "IntegrasiWS.asmx/Load_MaklumatBilTerperinci",
                    "method": 'POST',
            ...
            console.log('Before click event:', localStorage.getItem('redirectTerimaanUrl'));


            $('.btnKembali').click(async function () {
                var url = localStorage.getItem('redirectTerimaanUrl');
                console.log('Inside click event:', url);
        ...
        $('.btnCetak').click(async function () {
            ...
            $('#ddlBankUTeM').dropdown({
                ...
                fullTextSearch: true,
                apiSettings: {
                    url: '../Terimaan/TerimaWS.asmx/GetKodBankList',
                    method: 'POST',
                    dataType: "json",
                    contentType: 'application/json; charset=utf-8',
                    cache: false,
        ...
        async function AjaxGetRecordHdrJurnal(id) {
            ...
            try {
                ...
                const response = await fetch('../Integrasi/IntegrasiWS.asmx/LoadHdrInvoisPendahuluan', {
                    method: 'POST',
                    headers: {
        ...
        async function AjaxGetRecordHdrJurnalTerimaan(id) {
            ...
            try {
                ...
                const response = await fetch('../Integrasi/IntegrasiWS.asmx/LoadHdrInvoisTerimaan', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        async function AjaxGetRecordJurnal(id) {
            ...
            try {
                ...
                const response = await fetch('../Integrasi/IntegrasiWS.asmx/LoadRecordInvoisPendahuluan', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        async function AjaxGetRecordJurnalTerimaan(id) {
            ...
            try {
                ...
                const response = await fetch('../Integrasi/IntegrasiWS.asmx/LoadRecordInvoisTerimaan', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        async function setValueToRow_Transaksi(row, orderDetail) {
            ...
            $('#txtNoResit').html(orderDetail.No_Dok);
            ...
            lblCukai.html((orderDetail.Cukai ?? 0.00).toFixed(2));

            var cukai = $(row).find("td > .cukai");
            cukai.val((orderDetail.Cukai ?? 0.00).toFixed(2));
            ...
            lblDiskaun.html((orderDetail.Diskaun ?? 0.00).toFixed(2));

            var diskaun = $(row).find("td > .diskaun");
            diskaun.val((orderDetail.Diskaun ?? 0.00).toFixed(2));

            var lblAmount = $(row).find("td > .lblAmount_");
            lblAmount.html(orderDetail.Jumlah_anggaran.toFixed(2));

            var lblAmountBayar = $(row).find("td > .lblAmountBayar_");
            lblAmountBayar.html((orderDetail.Jumlah_Bayar ?? 0.00).toFixed(2));

            var amountBayar = $(row).find("td > .amountBayar");
            amountBayar.val((orderDetail.Jumlah_Bayar ?? 0.00).toFixed(2));

            var amount = $(row).find("td > .amount");
            amount.val(orderDetail.Jumlah_anggaran.toFixed(2));

            var JumLulus = orderDetail.Jum_Lulus;
            $(totalwoCukai).val(JumLulus.toFixed(2));
            document.getElementById('stickyJumlah').textContent = JumLulus.toLocaleString('en-US', { valute: 'USD', minimumFractionDigits: 2 })
            grandTotal.val(JumLulus.toLocaleString('en-US', { valute: 'USD', minimumFractionDigits: 2 }));
            total_sebenar.val(JumLulus.toLocaleString('en-US', { valute: 'USD', minimumFractionDigits: 2 }));
        ...
        async function setValueToRow_HdrJurnal(orderDetail) {
            ...
            console.log("Data Penghutang: ", $('#ddlPenghutang').val());
            ...
            selectObj_JenisTransaksi.append("<option value = '" + orderDetail.KodUrusniaga + "'>" + orderDetail.JNS_URUSNIAGA + "</option>")

            if (orderDetail.Kontrak == '1') {
        ...
        $('.btnSimpanPayment').click(async function () {
            ...
            if (result.Code == "00") {
                //alert("hi")
                var params = `scrollbars=no,resizable=no,status=no,location=no,toolbar=no,menubar=no,width=0,height=0,left=-1000,top=-1000`;
                window.open('<%=ResolveClientUrl("~/FORMS/AKAUN PENERIMA/Terimaan/CetakResit.aspx")%>?noResit=' + result.Payload.OrderIDResit, '_blank', params);
            }
        ...
        async function ajaxSaveOrder(id) {
            ...
            return new Promise((resolve, reject) => {
                $.ajax({
                    ...
                    url: 'IntegrasiWS.asmx/SavePaymentPendahuluan',
                    method: 'POST',
