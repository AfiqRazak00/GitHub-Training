...
<asp:Content ID="Content1" ContentPlaceHolderID="FormContents" runat="server">
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="transaction-table table-responsive">
                                    <table id="tblDataSenarai" class="table table-striped">
                                        <thead>
                                            <tr style="width: 100%">
                                                <th scope="col" style="width: 10%" id="lblColumnBil">No. Order</th>
                                                <th scope="col" style="width: 10%">No. Rujukan</th>
                                                <th scope="col" style="width: 10%">Tarikh Mohon</th>
                                                <th scope="col" style="width: 10%">Sesi</th>
                            ...
                            <div class="col-md-12">
                                <div class="form-row">
                                    <div class="form-group col-md-3">
                                        ...
                                        <label class="input-group__label" for="txtnoinv" id="lblModalBil">No. Order</label>
                                    </div>
                                    <div class="form-group col-md-3">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            if (status == "03") {
                $('#lblColumnBil').html('No. Order')
                $('#lblModalBil').html('No. Order')
            } else {
            ...
            tbl = $("#tblDataSenarai").DataTable({
                ...
                "ajax": {
                    ...
                    "data": function () {
                        const searchParams = new URLSearchParams(window.location.search);
                        var custcode = searchParams.get('KodPenghutang')
            ...
            $('#ddlBankUTeM').dropdown({
                ...
                fullTextSearch: true,
                apiSettings: {
                    url: '<%=ResolveClientUrl("~/FORMS/AKAUN PENERIMA/Terimaan/TerimaWS.asmx/GetKodBankList")%>',
                    method: 'POST',
                    dataType: "json",
                    contentType: 'application/json; charset=utf-8',
                    cache: false,
        ...
        $(document).ready(function () {
            ...
            $(".txtAmaunTerima").keyup(function () {
                ...
                var baki = (JumlahSebenar - amount).toLocaleString('en-US', { valute: 'USD', minimumFractionDigits: 2 });
        ...
        async function setValueToRow_Transaksi(row, orderDetail) {
            ...
            lblHarga.html(orderDetail.Kadar_Harga.toLocaleString('en-US', { valute: 'USD', minimumFractionDigits: 2 }));
            ...
            lblAmount.html(orderDetail.Jumlah.toLocaleString('en-US', { valute: 'USD', minimumFractionDigits: 2 }));

            var lblAmountBayar = $(row).find("td > .lblAmountBayar_");
            //lblAmountBayar.html(orderDetail.Jumlah_Bayar.toFixed(2));
            lblAmountBayar.html(orderDetail.Jumlah_Bayar.toLocaleString('en-US', { valute: 'USD', minimumFractionDigits: 2 }));
        ...
        async function setValueToRow_HdrJurnal(orderDetail) {
            ...
            $('#txtnoinv').val(orderDetail.No_Order)
            ...
            $(ddlUrusniaga).dropdown('set selected', orderDetail.Jns_Urusniaga);
            selectObj_JenisTransaksi.append("<option value = '" + orderDetail.Jns_Urusniaga + "'>" + orderDetail.But_Urusniaga + "</option>")
        ...
        $('.btnSimpanPayment').click(async function () {
            ...
            var newOrder = {
                order: {
                    No_Dok: '',
                    No_Rujukan: $('#txtnoinv').val(),
                    Kod_Penghutang: $('#ddlPenghutang').val(),
                    Jumlah_Sebenar: $('#txtAmaunTerima').val(),
                    Mod_Terima: $('#ddlModTerimaan').val(),
                    Kod_Bank: $('#ddlBankUTeM').val(),
                    Tkh_Daftar: $('#lblTkhMohon').val(),
                    Tkh_Dok: $('#tkhMula').val(),
                    Kontrak: $("input[name='inlineRadioOptions']:checked").val(),
                    Kod_Urusniaga: $('#ddlUrusniaga').val(),
                    Tujuan: $('#txtTujuan').val(),
                    Jumlah_Bayar: $('#total').val(),
                    OrderDetails: []
                }
            }
            $('.vot-carian-list').each(function (index, obj) {
                if (index > 0) {
                    ...
                    var orderDetail = {
                        No_Dok: '',
                        Kod_Kump_Wang: $('.hid-vot-list').eq(index).html(),
                        Kod_Operasi: $('.Hid-ptj-list').eq(index).html(),
                        Kod_PTJ: $('.Hid-kw-list').eq(index).html(),
                        Kod_Projek: $('.Hid-ko-list').eq(index).html(),
                        Kod_Vot: $('.Hid-kp-list').eq(index).html(),
                        Butiran: $('.details').eq(index).val(),
                        quantity: $('.quantity').eq(index).val(),
                        Kredit0: $('.price').eq(index).val(),
                        Jumlah_Diskaun: $('.diskaun').eq(index).val(),
                        Jumlah_Cukai: $('.cukai').eq(index).val(),
                        Debit: $('.amount').eq(index).val(),
        ...
        async function ajaxSaveOrder(id) {
            ...
            return new Promise((resolve, reject) => {
                $.ajax({
                    ...
                    url: 'IntegrasiWS.asmx/insertBil',
                    method: 'POST',
