<%@ Page Title="" Language="vb" Async="true" AutoEventWireup="false" MasterPageFile="~/NestedMasterPage2.master"
    CodeBehind="Integrasi_Kewangan.aspx.vb" Inherits="SMKB_Web_Portal.Integrasi_Kewangan" %>
                            ...
                            <div class="search-filter">
                                <div class="form-row justify-content-center">
                                    <div class="form-group row col-md-6">
                                        <label for="" class="col-sm-2 col-form-label" align="right">Carian:</label>
        ...
        <script type="text/javascript">
            function ShowDetails(id, status) {
                if (id !== "") {
                    if (status !== "") {
                        if (status == "NOTDONE") {
                            location.replace('<%=ResolveClientUrl("~/FORMS/AKAUN PENERIMA/Integrasi/Integrasi_Kewangan_Terperinci.aspx?kodsubmenu=btnMenu121404")%>&kodpenghutang=' + id + '&status=03', '_blank');
                        } else if (status == "DONE") {
                            location.replace('<%=ResolveClientUrl("~/FORMS/AKAUN PENERIMA/Integrasi/Integrasi_Kewangan_Terperinci.aspx?kodsubmenu=btnMenu121404")%>&kodpenghutang=' + id + '&status=04', '_blank');
                        }
                    }
                }
            }
            ...
            $(document).ready(function () {
                ...
                tbl = $("#tblDataSenarai").DataTable({
                    ...
                    "ajax": {
                        "url": "Integrasi.asmx/Load_SenaraiPenghutang",
                        "method": 'POST',
                ...
                $('.btnSearch').click(async function () {
                    ...
                    if (categoryFilter == "6") {
                        ...
                        const url = `<%=ResolveClientUrl("~/FORMS/AKAUN PENERIMA/Integrasi/Integrasi_Kewangan.aspx?kodsubmenu=btnMenu121802")%>&categoryFilter=${categoryFilter}&startDate=${startDate}&endDate=${endDate}`;

                        localStorage.setItem('redirectTerimaanUrl', url);
                        location.replace(url, '_blank');
                    } else {
                        const url = `<%=ResolveClientUrl("~/FORMS/AKAUN PENERIMA/Integrasi/Integrasi_Kewangan.aspx?kodsubmenu=btnMenu121802")%>&categoryFilter=${categoryFilter}`;
                        localStorage.setItem('redirectTerimaanUrl', url);
                        location.replace(url, '_blank');
                    }
