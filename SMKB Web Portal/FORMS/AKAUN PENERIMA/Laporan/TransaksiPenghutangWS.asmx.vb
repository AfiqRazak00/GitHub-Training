...
Public Class TransaksiPenghutangWS
    ...
    Private Function GetOrder_SenaraiLulusTransaksiJurnal() As DataTable
        ...
        Dim query As String = "SELECT kodpenghutang,Nama_Penghutang,No_Dok,No_Rujukan,Tujuan,FORMAT(Tkh_Lulus, 'dd/MM/yyyy') AS Tkh_Lulus,Kod_Status_Dok,Kod_Vot,DEBIT,KREDIT FROM
                            ...
                            INNER JOIN SMKB_Penghutang_Master AS C ON A.Kod_Penghutang=C.Kod_Penghutang
                            WHERE Kod_Status_Dok='06'
							UNION ALL
							SELECT B.Kod_Penghutang AS kodpenghutang, c.Nama_Penghutang ,A.No_Pelarasan AS No_Dok, B.No_Rujukan,A.Tujuan,A.Tkh_Pelarasan AS Tkh_Lulus,A.Kod_Status_Dok, Kod_Vot, 
							CASE WHEN LEFT(A.No_Pelarasan,2) = 'ND' THEN A.Jumlah_Besar ELSE '0.00' END AS DEBIT, CASE WHEN LEFT(A.No_Pelarasan,2) = 'NK' THEN A.Jumlah_Besar ELSE '0.00' END AS KREDIT
							FROM SMKB_Bil_Adj_Hdr A
							INNER JOIN SMKB_Bil_Hdr B ON A.No_Bil=B.No_Bil
							INNER JOIN SMKB_Penghutang_Master AS C ON B.Kod_Penghutang=C.Kod_Penghutang
							INNER JOIN SMKB_Bil_Adj_Dtl D ON A.No_Pelarasan=D.No_Pelarasan
							WHERE A.Kod_Status_Dok='02') AS A
                            ORDER BY Tkh_Lulus"
    ...
    Private Function GetOrder_SenaraiResitBatal() As DataTable
        ...
        Dim query As String = "SELECT A.No_Dok as No_Dok, FORMAT(Tkh_Daftar, 'dd/MM/yyyy') as TkhDaftar,FORMAT(Tkh_Batal, 'dd/MM/yyyy') as TkhBatal, 
                               ...
                               (SELECT MS01_Nama FROM VPeribadi WHERE MS01_NoStaf=Staf_Terima) AS Staf_Terima 
