...
Public Class SenaraiBilBerulang
    ...
    Public Function SaveBilBerulang(order As Bil_Hdr) As String
        ...
        If order.OrderID = "" Then
            ...
            order.tkhBil = CDate(order.tkhBil).ToString("yyyy-MM-dd")
            order.TkhMula = CDate(order.TkhMula).ToString("yyyy-MM-dd")
            order.TkhTamat = CDate(order.TkhTamat).ToString("yyyy-MM-dd")
            If InsertBilHdr(order.OrderID, order.PenghutangID, order.TkhMula, order.TkhTamat, order.Kontrak, order.JenisUrusniaga, order.Tujuan, order.Jumlah, order.tkhBil, order.tempoh, order.tempohbyrn, order.norujukan, "02") <> "OK" Then
    ...
    Private Function GetOrder_SenaraiBilBerulang(category_filter As String, tkhMula As DateTime, tkhTamat As DateTime) As DataTable
        ...
        Dim query As String = "SELECT A.No_Bil as No_Invois,B.Nama_Penghutang,C.Butiran as UrusNiaga,A.Tujuan,FORMAT(A.Tkh_Bil, 'dd/MM/yyyy') As TkhBil,FORMAT(A.Tkh_Mohon, 'dd/MM/yyyy') AS TKHMOHON,
                                ...
                                WHERE  A.Status='1' AND A.Kontrak = '1' AND No_Bil NOT IN (SELECT No_Bil FROM SMKB_Bil_Kontrak) AND A.Kod_Status_Dok='03' " & tarikhQuery
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function LoadOrderRecord_SenaraiBilDijana(category_filterJana As String, isClickedJana As Boolean, tkhMula As DateTime, tkhTamat As DateTime) As String
        Dim resp As New ResponseRepository

        If isClickedJana = False Then
            Return JsonConvert.SerializeObject(New DataTable)
        End If

        dt = GetOrder_SenaraiBilDijana(category_filterJana, tkhMula, tkhTamat)
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Private Function GetOrder_SenaraiBilDijana(category_filterJana As String, tkhMulaBil As DateTime, tkhTamatBil As DateTime) As DataTable
        Dim db = New DBKewConn
        Dim tarikhQuery As String = ""
        Dim param As List(Of SqlParameter) = New List(Of SqlParameter)()

        If category_filterJana = "1" Then 'Harini
            tarikhQuery = " AND CAST(A.Tkh_Mohon AS DATE) = CAST(CURRENT_TIMESTAMP AS DATE) "
        ElseIf category_filterJana = "2" Then 'Semalam
            tarikhQuery = " AND CAST(A.Tkh_Mohon AS DATE) = CAST(DATEADD(day, -1, CURRENT_TIMESTAMP) AS DATE) "
        ElseIf category_filterJana = "3" Then 'seminggu
            tarikhQuery = " AND A.Tkh_Mohon >= DATEADD(day, -7, CURRENT_TIMESTAMP) AND A.Tkh_Mohon < CURRENT_TIMESTAMP "
        ElseIf category_filterJana = "4" Then '30 hari
            tarikhQuery = " AND A.Tkh_Mohon >= DATEADD(day, -30, CURRENT_TIMESTAMP) AND A.Tkh_Mohon < CURRENT_TIMESTAMP "
        ElseIf category_filterJana = "5" Then '60 hari
            tarikhQuery = " AND A.Tkh_Mohon >= DATEADD(day, -60, CURRENT_TIMESTAMP) AND A.Tkh_Mohon < CURRENT_TIMESTAMP "
        ElseIf category_filterJana = "6" Then 'custom
            tarikhQuery = " AND A.Tkh_Mohon >= @TkhMula AND A.Tkh_Mohon <= @TkhTamat "
            param.Add(New SqlParameter("@TkhMula", tkhMulaBil))
            param.Add(New SqlParameter("@TkhTamat", tkhTamatBil))
        End If
        ...
        Dim query As String = "SELECT A.No_Bil as No_Bil,B.Nama_Penghutang,C.Butiran as UrusNiaga,A.Tujuan,
                               ...
                               END
                               AS Penyedia,A.No_Staf_Penyedia, (SELECT COUNT(No_Item) FROM SMKB_Bil_Dtl WHERE SMKB_Bil_Dtl.No_Bil = A.No_Bil ) AS NO_ITEM,A.Kod_Status_Dok
                               FROM SMKB_Bil_Hdr A
                               inner JOIN SMKB_Penghutang_Master B ON A.Kod_Penghutang=B.Kod_Penghutang
                               inner JOIN SMKB_Kod_Urusniaga C ON A.Kod_Urusniaga=C.Kod_Urusniaga
                               ...
                               INNER JOIN SMKB_Bil_Kontrak F ON A.No_Bil=F.No_Bil
