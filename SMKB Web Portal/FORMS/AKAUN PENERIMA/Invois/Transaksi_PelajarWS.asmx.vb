...
Public Class Transaksi_PelajarWS
    ...
    Public Function GetKategoriPenghutang(kod As String) As DataTable
        Dim db = New DBKewConn
        Dim query As String = "SELECT Kod_Detail AS value, Butiran AS text
                                FROM SMKB_Lookup_Detail 
                                WHERE Kod='0152' AND Kod_Detail IN ('PL','PG','PH')"
        Return db.Read(query)
    End Function
    ...
    Private Function GetSesi(kod As String) As DataTable
        ...
        Dim query As String = "SELECT KodSesi_Sem AS value, KodSesi_Sem AS text 
                                FROM SMP_SesiPengajian WHERE YEAR(TrkMula_Sem) <= year(getdate()) and YEAR(TrkMula_Sem) > year(getdate())-3"
        'WHERE Keaktifan='1'
        'AND Stat_Skrg='1'
        Return db.Read(query)
    End Function
    ...
    Private Function Getfakulti(kod As String) As DataTable
        Dim db = New DBKewConn
        Dim query As String = "SELECT CONCAT(KodPejabat, '0000' ) AS value, CONCAT(KodPejabat, '0000' ) +'-'+ Pejabat AS text
                                FROM VPEJABAT 
