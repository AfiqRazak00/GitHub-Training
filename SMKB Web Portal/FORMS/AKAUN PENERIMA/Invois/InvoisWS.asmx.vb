...
Public Class InvoisWS
    ...
    '    Dim query As String = "SELECT top 10 CONCAT(a.Kod_Kump_Wang, ' - ', REPLACE(kw.Butiran, 'KUMPULAN WANG', 'KW'), ', ', a.Kod_Operasi, ' - ', ko.Butiran, ', ',a.kod_PTJ , ' - ' , mj.Pejabat, ', ',a.Kod_Vot, ' - ', vot.Butiran, ', ', a.Kod_Projek, ' - ',kp.Butiran) AS text,
    '                a.Kod_Vot AS value ,
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Async Function SubmitOrdersKaunter(order As Order_Resit) As Tasks.Task(Of String)
        Dim resp As New ResponseRepository
        resp.Success("Data telah disimpan")
        Dim success As Integer = 0
        Dim JumRekod As Integer = 0
        ...
        If order.OrderID = "" Then
            ...
            Else
                order.Jumlah = order.Jumlah
            End If
            If InsertNewOrderKaunter(order.OrderID, order.PenghutangID, order.TkhMula, order.TkhTamat, order.Kontrak, order.JenisUrusniaga, order.Tujuan, order.Jumlah, order.tkhBil, order.tempoh, order.tempohbyrn, order.norujukan, "03") <> "OK" Then
                resp.Failed("Gagal Menyimpan order")
                Exit Function
            End If
        ...
        'For Each mklmtBayaran As OrderDetail_MklmtBayaran In order.MklmtBayaran
        '    If mklmtBayaran.NoDok = "" Then
        '        mklmtBayaran.NoDok = order.OrderIDResit
        '        'mklmtBayaran.NoRujukan = order.OrderID
        '        If InsertKaunterMklmtBayaran(mklmtBayaran) <> "OK" Then
        '            resp.Failed("Gagal Menyimpan Maklumat Bayaran")
        '            Exit Function
        '        Else
        '            JumRekod += 1
        '            success += 1
    ...
    'insertNewOrder (Bill_Hdr) with updated lulus ?
    'insertOrderdetails with updated Lulus
    'insertresit (TerimaHdr) with updated Lulus DONE
    'insert
    ...
    Private Function InsertNewOrderKaunter(orderid As String, penghutangid As String, TkhMula As String, TkhTamat As String, Kontrak As String, JenisUrusniaga As String, Tujuan As String, Jumlah As String, tkhBil As String, tempoh As String, tempohbyrn As String, norujukan As String, statusdok As String)
        Dim db As New DBKewConn
        ...
        VALUES (@No_Bil, YEAR(getdate()), getdate(), @Jenis, @NoStaf, @statusdok ,'1',@Kod_Penghutang,@Tkh_Mula,@Tkh_Tamat,@Kontrak,@Kod_Urusniaga,@Tujuan,@Jumlah,@NoStafPenyedia,@tkhbil,@tempoh,@tempohbyrn, @norujukan, getdate(), @NoStaf, @flagLulus)"
        Dim param As New List(Of SqlParameter)
    ...
    Private Function InsertNewOrderResit(orderid As String, orderidresit As String, penghutangid As String, TkhMula As String, TkhTamat As String, Kontrak As String, JenisUrusniaga As String, Tujuan As String, Jumlah As String, tkhBil As String, tempoh As String, tempohbyrn As String, norujukan As String, modterima As String, bankutem As String, statusdok As String)
        ...
        Dim query As String = "INSERT INTO SMKB_Terima_Hdr
                              (No_Dok, Kod_Penghutang, No_Rujukan,Tujuan,Kod_Urusniaga,Mod_Terima,Tkh_Daftar,Staf_Terima,Jumlah_Sebenar,Jumlah_Bayar,
                               ...
                               VALUES 
                              (@noresit,@Kod_Penghutang,@No_Bil,@Tujuan,@Kod_Urusniaga,@modterima,@tarikh,@No_Staf,@Jumlah,@jumlahterima,
							   @No_Staf,@FlagClosing, @FlagLulus, @No_Staf,@tarikh,@tarikh, @tarikh,@kodbank,'TK',@status,@flag)"
        Dim param As New List(Of SqlParameter)
    ...
    Public Function InsertOrderDetail(orderDetail As OrderDetail_inv)
        ...
        VALUES( @no_bil , @no_item, @Kod_Kump_Wang, @Kod_Operasi , @Kod_PTJ , @Kod_Projek  , @Kod_Vot, @Perkara, @Kuantiti, @Kadar_Harga, @Jumlah, @Tahun, '1', @Diskaun, @Cukai, @StatusBayar)    "
        Dim param As New List(Of SqlParameter)
        ...
        param.Add(New SqlParameter("@Tahun", Year(Now())))
        param.Add(New SqlParameter("@StatusBayar", "BB"))
    ...
    'Function Insert() Maklumat Bayaran
    Public Function InsertKaunterMklmtBayaran(mklmtBayaran As OrderDetail_MklmtBayaran)
        Dim db = New DBKewConn
        Dim query As String = "INSERT INTO SMKB_Terima_Transaksi (No_Dok, No_Rujukan, Tarikh, Jumlah, Kod_Bank, Cawangan_Bank, Kod_Jenis_Cek, Status) VALUES (@NoDok, @NoRujukan, @Tarikh, @Jumlah, @KodBank, @CwgnBank, @JenCek, @status)"
        Dim param As New List(Of SqlParameter)

        param.Add(New SqlParameter("@NoDok", mklmtBayaran.NoDok))
        param.Add(New SqlParameter("@NoRujukan", mklmtBayaran.NoRujukan))
        param.Add(New SqlParameter("@Tarikh", mklmtBayaran.TKhBayar))
        param.Add(New SqlParameter("@Jumlah", mklmtBayaran.JumBayar))
        param.Add(New SqlParameter("@KodBank", mklmtBayaran.KodBank))
        param.Add(New SqlParameter("@CwgnBank", mklmtBayaran.CwgnBank))
        param.Add(New SqlParameter("@JenCek", mklmtBayaran.JenCek))
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function LoadHdrInvoisKaunter(ByVal id As String) As String
        Dim resp As New ResponseRepository

        dt = GetHdrInvoisKaunter(id)
        resp.SuccessPayload(dt)

        Return JsonConvert.SerializeObject(resp.GetResult())
    End Function

    Private Function GetHdrInvoisKaunter(id As String) As DataTable
        Dim db = New DBKewConn

        Dim query As String = "SELECT A.No_Bil, G.No_Dok, A.Kod_Penghutang, B.Nama_Penghutang, B.Emel, B.Kategori_Penghutang, D.Butiran as Butiran_Kategori, A.Kod_Urusniaga, C.Butiran, A.Kontrak, A.Tujuan, A.Kod_Status_Dok,G.Mod_Terima, G.Kod_Bank,
                                CASE WHEN Tkh_Mula <> '' THEN FORMAT(Tkh_Mula, 'yyyy-MM-dd') END AS TkhMula,
                                CASE WHEN Tkh_Tamat <> '' THEN FORMAT(Tkh_Tamat, 'yyyy-MM-dd') END AS TkhTamat,Tkh_Mula,Tkh_Tamat,Tkh_Bil,A.No_Rujukan,Tempoh_Kontrak,Jenis_Tempoh,F.Butiran AS JenisTempoh,
								(SELECT Butiran FROM SMKB_Mod_Bayar H WHERE G.Mod_Terima = H.Mod_Bayar AND Status='1') As ButiranMod,
								(SELECT Nama_Bank FROM SMKB_Bank_Master I WHERE G.Kod_Bank = I.Kod_Bank) As BankUtem
                                FROM SMKB_Bil_Hdr A
                                LEFT JOIN SMKB_Penghutang_Master B ON A.Kod_Penghutang=B.Kod_Penghutang
                                LEFT JOIN SMKB_Kod_Urusniaga C ON A.Kod_Urusniaga=C.Kod_Urusniaga
                                LEFT JOIN SMKB_Lookup_Detail D ON B.Kategori_Penghutang = D.Kod_Detail
                                LEFT JOIN SMKB_Lookup_Detail F ON A.Jenis_Tempoh=F.Kod_Detail AND F.Kod='AR09'
								INNER JOIN SMKB_Terima_Hdr G ON A.No_Bil = G.No_Rujukan
                                WHERE G.No_Dok = @No_Invois AND A.Status='1' AND D.Kod='0152'"
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function LoadRecordInvoiskaunter(ByVal id As String) As String
        Dim resp As New ResponseRepository

        dt = GetTransaksiInvoisKaunter(id)
        resp.SuccessPayload(dt)

        Return JsonConvert.SerializeObject(resp.GetResult())
    End Function

    Private Function GetTransaksiInvoisKaunter(kod As String) As DataTable
        Dim db = New DBKewConn
        ...
        Perkara ,Kuantiti, Kadar_Harga, Jumlah, Diskaun, Cukai
        from SMKB_Bil_Dtl as a
		INNER JOIN SMKB_Terima_Hdr B On a.No_Bil = B.No_Rujukan
        where B.No_Dok = @No_Invois
        and B.status = 1
        order by No_Item"
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function LoadDtlTxn(ByVal id As String) As String
        Dim resp As New ResponseRepository

        dt = GetDtlTransaksiDtl(id)
        resp.SuccessPayload(dt)

        Return JsonConvert.SerializeObject(resp.GetResult())
    End Function

    Private Function GetDtlTransaksiDtl(id As String) As DataTable
        Dim db = New DBKewConn

        Dim query As String = "SELECT A.No_Dok As No_Dok, A.No_Rujukan As No_Rujukan, A.Kod_Bank as KodBank, A.Kod_Jenis_Cek as KodCek, 
                               CASE WHEN Tarikh <> '' THEN FORMAT(Tarikh, 'yyyy-MM-dd') END AS Tarikh,
                               A.Jumlah,
                               (SELECT  Butiran FROM SMKB_Lookup_Detail lp WHERE lp.Kod_Detail = A.Kod_Bank AND lp.Kod = '0097' ) As ButiranBank,
                               A.Cawangan_Bank, 
                               (SELECT Butiran FROM SMKB_Lookup_Detail lp WHERE Kod = 'AR02' AND lp.Kod_Detail = A.Kod_Jenis_Cek AND Status='1') As JenCek
                               FROM SMKB_Terima_Transaksi A
                               WHERE A.No_Dok = @No_Invois
                               AND A.Status = '1'"
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function LoadOrderRecord_SenaraiTransaksiResit(category_filter As String, isClicked As Boolean, tkhMula As DateTime, tkhTamat As DateTime) As String
        Dim resp As New ResponseRepository
        ...
        dt = GetOrder_SenaraiTransaksiResit(category_filter, tkhMula, tkhTamat)
        'resp.SuccessPayload(dt)
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Private Function GetOrder_SenaraiTransaksiResit(category_filter As String, tkhMula As DateTime, tkhTamat As DateTime) As DataTable
        Dim db = New DBKewConn
        Dim tarikhQuery As String = ""
        Dim param As List(Of SqlParameter) = New List(Of SqlParameter)()
        ...
        ElseIf category_filter = "6" Then 'custom
            tarikhQuery = " AND Tkh_Daftar >= @tkhMula AND Tkh_Daftar <= @TkhTamat "
            param.Add(New SqlParameter("@tkhMula", tkhMula))
            param.Add(New SqlParameter("@TkhTamat", tkhTamat))
        End If
        ...
        Dim query As String = "SELECT A.No_Dok as No_Dok ,E.Nama_Penghutang,F.Butiran as UrusNiaga,A.Tujuan, A.Tkh_Daftar,
                               CASE WHEN A.Tkh_Daftar <> '' THEN FORMAT(a.Tkh_Daftar, 'dd/MM/yyyy') END AS Tkh_Dok,
                               CASE WHEN Tkh_Lulus <> '' THEN FORMAT(Tkh_Lulus, 'dd/MM/yyyy') END AS Tkh_Lulus,
                               A.Jumlah_Bayar,F.Butiran AS STATUS_RESIT,
                               CASE WHEN A.Staf_Terima  <> '' THEN D.MS01_Nama END
                               AS Penyedia,A.Staf_Terima, (SELECT COUNT(No_Item) FROM SMKB_Terima_Dtl WHERE SMKB_Terima_Dtl.No_Dok = A.No_Dok ) AS NO_ITEM
                               FROM SMKB_Terima_Hdr A, SMKB_Terima_Dtl B, SMKB_Terima_Transaksi C, VPeribadi D,
                               SMKB_Penghutang_Master E, SMKB_Kod_Urusniaga F, SMKB_Kod_Status_Dok G
                               WHERE A.No_Dok=B.No_Dok AND A.No_Dok=C.No_Dok
                               AND A.Staf_Terima=D.MS01_NoStaf AND A.Kod_Penghutang=E.Kod_Penghutang
                               AND A.Kod_Urusniaga=F.Kod_Urusniaga AND A.Kod_Status_Dok=G.Kod_Status_Dok AND Kod_Modul='12' AND A.Flag_Kaunter='1'
                               AND  A.Status ='1' " & tarikhQuery & "
                               ORDER BY Tkh_Dok , No_Dok"
    ...
    <System.Web.Services.WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function GetKodBankList(ByVal q As String) As String

        Dim tmpDT As DataTable = GetLoadKodBankList(q)
        Return JsonConvert.SerializeObject(tmpDT)
    End Function
    ...
    Private Function GetLoadKodBankList(kod As String) As DataTable
        ...
        If kod <> "" Then
            ...
            query &= " WHERE (Kod_Bank LIKE '%' + @kod + '%' OR Nama_Bank LIKE '%' + @kod2 + '%') "
            param.Add(New SqlParameter("@kod", kod))
            param.Add(New SqlParameter("@kod2", kod))
        End If
    ...
    <System.Web.Services.WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function LoadJenisCekList(ByVal q As String) As String

        Dim tmpDT As DataTable = GetJenisCekList(q)
        Return JsonConvert.SerializeObject(tmpDT)
    End Function

    Private Function GetJenisCekList(kod As String) As DataTable
        Dim db = New DBKewConn
        Dim query As String = "SELECT Kod_Detail AS value,Butiran AS text FROM SMKB_Lookup_Detail
                            WHERE Kod = @kodMaster AND Status='1'"
        Dim param As New List(Of SqlParameter)
        If kod <> "" Then
            query &= " AND (Kod_Detail LIKE '%' + @kod + '%' OR Butiran LIKE '%' + @kod2 + '%') "
            param.Add(New SqlParameter("@kod", kod))
            param.Add(New SqlParameter("@kod2", kod))
        End If
        param.Add(New SqlParameter("@kodMaster", "AR02"))
        Return db.Read(query, param)
    End Function
    ...
    Private Async Sub EmelStaf(orderID As String)
        ...
        Dim fullName As String = "AIDA HAZIRAH BINTI ABDUL HAMID"
        Dim email As String = "aida.hazirah@utem.edu.my"
        Dim NoStaff = "02642"

        Dim KodSubMenu = "120102" 'identify kod sub menu skrin sokong/lulus ikut modul msing2
        ...
        If db.fInsertCommand(strSqlButiran, paramSqlBtrn) > 0 Then
            ...
            Dim url As String = "http://localhost:1558/SMKBNet/loginsmkb?id=" + id

            'Send the New password to the user's email
            Dim subject As String = "UTeM - Sistem Maklumat Kewangan Bersepadu"
            Dim body As String = "Kelulusan Transaksi Bil " &
                         "<br><br>" &
                         vbCrLf & "Assalamualaikum Dan Salam Sejahtera " & fullName & "," &
                         "<br><br>" &
                         vbCrLf & "Memerlukan kelulusan anda untuk transaksi bil. " &
                         "<br><br>" &
                         vbCrLf & "Sila klik di link ini untuk menyemak transaksi bil untuk kelulusan melalui <a href=" + url + ">" + url + "</a>" &
                         "<br>" &
                         "<br><br>" &
            ...
            ''Notification API
            ' API endpoint URL
            Dim apiUrl As String = "https://devmobile.utem.edu.my/smkbnotification/api/notification/smkb/SISTEM MAKLUMAT KEWANGAN BERSEPADU/Kelulusan Bil.Memerlukan kelulusan anda untuk kelulusan bil./" + id + "/" + NoStaff
