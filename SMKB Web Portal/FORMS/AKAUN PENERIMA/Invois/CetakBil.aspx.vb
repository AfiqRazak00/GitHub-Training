...
Public Class CetakBil
    ...
    Private Sub fBindTransaksi(bil As String)
        If bil <> "" Then
            ...
            Dim nobil = Left(bil, 1)
            If nobil = "I" Then
                strSql = $"SELECT A.No_Order,A.Kod_Penghutang,UPPER(B.Nama_Penghutang) AS Nama_Penghutang,(CASE WHEN A.Pengirim='PINJ' THEN '1' WHEN  A.Pengirim='ADV' THEN '10' END) as KodUrusNiaga,(CASE WHEN A.Pengirim='PINJ' THEN 'PINJAMAN' WHEN  A.Pengirim='ADV' THEN 'PENDAHULUAN' END)  AS Butiran,'0' AS Kontrak,A.Pengirim_Mohon AS Tujuan,
                                CASE WHEN Tkh_Transaksi <> '' THEN FORMAT(Tkh_Transaksi, 'dd/MM/yyyy') END AS Tkh_Lulus,
                                (SELECT REPLACE(FORMAT (SUM(a.Jumlah), 'c', 'en-MY'),'RM','') FROM AR_Order_Dtl as a where No_Order = '{bil}' and status = 1 ) AS Jumlah, 
                                '0' AS Diskaun,
                                '0' AS Cukai,
                                REPLACE(FORMAT (a.Jumlah_Keseluruhan, 'c', 'en-MY'),'RM','') as JumlahSebenar,
                                ...
                                WHERE No_Order = '{bil}'  AND A.Status='1'"
                ...
                Using dt = ds.Tables(0)
                    If dt.Rows.Count > 0 Then
                        ...
                        txtnobil.InnerText = dt.Rows(0)("No_Order").ToString
            ...
            Else
                strSql = $"SELECT A.No_Bil,A.Kod_Penghutang,UPPER(B.Nama_Penghutang) AS Nama_Penghutang,A.Kod_Urusniaga,C.Butiran, A.Kontrak,A.Tujuan,Kod_Status_Dok,
                                ...
                                (SELECT REPLACE(FORMAT (SUM(a.Jumlah), 'c', 'en-MY'),'RM','') FROM SMKB_Bil_Dtl as a where No_Bil = '{bil}' and status = 1 ) AS Jumlah, 
                                ...
                                REPLACE(FORMAT (Jumlah, 'c', 'en-MY'),'RM','') as JumlahSebenar,
								UPPER(B.Alamat_1) +','+UPPER(B.Alamat_2)  AS ALAMAT_1,
								B.Poskod+','+UPPER(G.Butiran)  AS ALAMAT_2,
								UPPER(D.Butiran)+','+UPPER(F.Butiran) AS ALAMAT_3,B.Emel,B.Tel_Bimbit
                                FROM SMKB_Bil_Hdr A
    ...
    Private Sub fBindTransaksi_Details(bil As String)
        If bil <> "" Then
            ...
            Dim nobil = Left(bil, 1)
            If nobil = "I" Then
                strSql = $"SELECT A.No_Order,(SELECT Butiran FROM SMKB_Vot WHERE Kod_Vot=A.Kod_Vot) AS Perkara ,'1' AS Kuantiti,REPLACE(FORMAT (Jumlah, 'c', 'en-MY'),'RM','') AS Kadar_Harga, REPLACE(FORMAT (Jumlah, 'c', 'en-MY'),'RM','') AS Jumlah,'0.00' AS Diskaun,'0.00' AS Cukai
                                FROM AR_Order_Dtl A, SMKB_COA_Master B
                                WHERE A.Kod_Kump_Wang=B.Kod_Kump_Wang AND A.Kod_Operasi=B.Kod_Operasi 
								AND A.Kod_Projek=B.Kod_Projek AND A.Kod_Vot=B.Kod_Vot
                                AND B.Kod_PTJ='500000' AND No_Order = '{bil}'
                ...
                End If
            Else
                strSql = $"SELECT a.No_Bil,a.Perkara AS Perkara,'1' AS Kuantiti, REPLACE(FORMAT (Jumlah, 'c', 'en-MY'),'RM','') AS Kadar_Harga, REPLACE(FORMAT (Jumlah, 'c', 'en-MY'),'RM','') AS Jumlah,'0.00' AS Diskaun,'0.00' AS Cukai
                                FROM SMKB_Bil_Dtl A, SMKB_COA_Master B
                                WHERE A.Kod_Kump_Wang=B.Kod_Kump_Wang AND A.Kod_Operasi=B.Kod_Operasi 
								AND A.Kod_Projek=B.Kod_Projek AND A.Kod_PTJ=B.Kod_PTJ AND A.Kod_Vot=B.Kod_Vot
                                AND No_Bil = '{bil}'
                                AND A.status = 1 AND B.Status='1'
                                ORDER BY No_Item"
                Dim ds = dbconn.fSelectCommand(strSql)
