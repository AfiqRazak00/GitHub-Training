<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/NestedMasterPage2.master" CodeBehind="PembatalanPT.aspx.vb" Inherits="SMKB_Web_Portal.PembatalanPT" %>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/moment.js/2.29.1/moment.min.js" crossorigin="anonymous"></script>
            ...
            <div>
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalCenterTitle">Pembatalan Pesanan Tempatan</h5>
                    </div>
            ...
            <div class="modal-dialog modal-dialog-centered modal-xl modal-dialog-scrollable" role="document">
                <div class="modal-content">
                    <div class="modal-header modal-header--sticky">
                        <h5 class="modal-title">PEMBATALAN PESANAN TEMPATAN</h5>
 ...
 <%--                                   <div class="form-group col-md-6">
                            ...
                            <div class="col-md-12">
                                <div class="form-row">
                                   <div class="form-group col-md-6">
                                    ...
                                    <input class="" id="kodPemiutang" placeholder="&nbsp;" name="Kod Pemiutang" type="hidden" />
                                    <input class="" id="status_Dok" placeholder="&nbsp;" name="Status Dok" type="hidden" />
                     ...
                     <div class="modal-footer modal-footer--sticky" style="padding: 0px!important">                        
                        <button type="button" class="btn btn-setsemula btnBatalPtSahaja" data-toggle="tooltip" data-placement="bottom">Batal PT Sahaja</button>
                        <button type="button" class="btn btn-success btnBatalPtPermohonan" data-toggle="tooltip" data-placement="bottom">Batal PT dan Permohonan</button>                     
                    </div>
        ...
        <div class="modal fade" id="saveConfirmationModal12" tabindex="-1" role="dialog" aria-labelledby="saveConfirmationModalLabel" aria-hidden="true">
            <div class="modal-dialog " role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="saveConfirmationModalLabel12">Lulus Pesanan Tempatan</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-danger" data-dismiss="modal">Tidak</button>
                        <button type="button" class="btn btn-secondary" id="batalPT">Ya</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function () {
                ...
                tbl = $("#tblSenaraiPerolehan").DataTable({
                    ...
                    "ajax": {
                        url: '<%= ResolveClientUrl("~/FORMS/PEROLEHAN/PESANAN TEMPATAN/PesananTempatanWS.asmx/Load_BatalPtPermohonan") %>',
                        method: 'POST',
                    ...
                    "rowCallback": function (row, data) {
                        ...
                        $(row).on("click", function () {
                            ...
                            $("#kodPemiutang").val(data.Kod_Pemiutang);
                            $("#status_Dok").val(data.Status_Dok);
            ...
            var rowDataArray = [];
            $('.btnBatalPtSahaja').off('click').on('click', async function () {
                var msg = "Anda pasti untuk membatal pesanan tempatan ini?";
                $('#confirmationMessage12').text(msg);
                $('#transaksi').modal('hide');
                $('#saveConfirmationModal12').modal('show');

                $('#batalPT').off('click').on('click', async function () {
                    $('#saveConfirmationModal12').modal('hide');

                    tblSpek.rows().every(function (index, element) {
                    ...
                    var txtNoMohon = $('#txtNoMohon').val()
                    var NoPt = $('#txtNoPT').val()
                    var kPemiutang = $('#kodPemiutang').val()
                    var status_Dok = $('#status_Dok').val()


                    try {
                        var result = (await ajaxBatalPtSahaja(txtNoMohon, NoPt, kPemiutang, status_Dok, rowDataArray));
            ...
            async function ajaxBatalPtSahaja(txtNoMohon, NoPt, kPemiutang, status_Dok, rowDataArray) {
                return new Promise((resolve, reject) => {
                    $.ajax({
                        "url": '<%= ResolveClientUrl("~/FORMS/PEROLEHAN/PESANAN TEMPATAN/PesananTempatanWS.asmx/BatalPtSahaja") %>',
                        method: 'POST',

                        data: JSON.stringify({
                            "txtNoMohon": txtNoMohon,
                            "NoPt": NoPt,
                            "kPemiutang": kPemiutang,
            ...
            var rowDataArray = [];
            $('.btnBatalPtPermohonan').off('click').on('click', async function () {
                var msg = "Anda pasti untuk membatal pesanan tempatan ini?";
                ...
                $('#registerPT').off('click').on('click', async function () {
                    ...
                    try {
                        var result = (await ajaxLulusPesananTempatan(txtNoMohon, NoPt, kPemiutang, status_Dok,rowDataArray));
                        result = JSON.parse(result.Result);
            ...
            async function ajaxLulusPesananTempatan(txtNoMohon, NoPt, kPemiutang, status_Dok, rowDataArray) {
                return new Promise((resolve, reject) => {
                    $.ajax({
                        "url": '<%= ResolveClientUrl("~/FORMS/PEROLEHAN/PESANAN TEMPATAN/PesananTempatanWS.asmx/BatalPesananTempatan") %>',
                        method: 'POST',
            ...
            tblSpek = $("#tblDataPerolehanDtl").DataTable({
                ...
                "ajax": {
                    "url": '<%= ResolveClientUrl("~/FORMS/PEROLEHAN/PESANAN TEMPATAN/PesananTempatanWS.asmx/Load_PtUniversiti") %>',
                    type: 'POST',
                    "contentType": "application/json; charset=utf-8",
                    "dataType": "json",
                    data: function (d) {
