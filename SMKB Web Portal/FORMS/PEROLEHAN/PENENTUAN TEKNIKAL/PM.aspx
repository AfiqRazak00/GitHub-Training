<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/NestedMasterPage2.master" CodeBehind="PM.aspx.vb" Inherits="SMKB_Web_Portal.PM" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="FormContents" runat="server">
    ...
    <script type="text/javascript">
        ...
        const txtMasa = document.getElementById('txtMasa');
        const txtTempat = document.getElementById('txtTempat');
        const selectJawatankuasa = document.getElementById('JawatankuasaData');
        const btnSimpan = document.getElementById('btnSimpanInfo');
        ...
        function checkInputs() {
            const dropTarget = document.getElementById("dropTarget");
            const draggableElements = dropTarget.querySelectorAll(".draggable");
            const tarikhValue = txtTarikh.value.trim();
            const masaValue = txtMasa.value.trim();
            const tempatValue = txtTempat.value.trim();

            // Flag to check if draggable elements have data
            let draggableHasData = false;
            draggableElements.forEach(element => {
                if (element.textContent.trim() !== '') {
                    draggableHasData = true;
                }
            });

            //console.log('tarikhValue', tarikhValue);
            //console.log('masaValue', masaValue);
            //console.log('draggableElements', draggableElements.length);
            //console.log('draggableHasData', draggableHasData);

            // If all fields have data, enable the button
            if (tarikhValue !== '' && masaValue !== '' && tempatValue !== '' && draggableHasData && KodJawatanKuas !== '') {
        ...
        txtMasa.addEventListener('input', checkInputs);
        txtTempat.addEventListener('input', checkInputs);
        dropTarget.addEventListener("drop", checkInputs);
        selectJawatankuasa.addEventListener('change', checkInputs);
    ...
    <script type="text/javascript">
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            $(document).ready(async function () {
                ...
                // Load data table on document ready
                await loadDataTable();
                ...
                $('#btnSimpanInfo').off('click').on('click', async function () {
                    ...
                    var msg = "Anda pasti ingin daftar maklumat Mesyuarat ini? ";
                    ...
                    $('#confirmSaveButton1').off('click').on('click', async function () {
                        ...
                        if (result.Status === true) {
                            ...
                            console.log("masuk=", result.Message)
                            // Reload data after submit
                            $('#txtTarikh').val('');
                            $('#txtMasa').val('');
                            $('#txtTempat').val('');
                            $('#JawatankuasaData').val('');
                            //location.reload();
                        ...
                        draggableElements.forEach(async (element, index) => {
                            ...
                            if (result.Status === true) {
                                showModal1("Success", result.Message, "success");
                                const callback = JSON.parse(await getDataTable())
                        ...
                        var kodSubmanu = "020201"
                        var NameSubMenu = "Penentuan Teknikal"
