<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/NestedMasterPage2.master" CodeBehind="Daftar_Pembuka_PTj.aspx.vb" Inherits="SMKB_Web_Portal.Daftar_Pembuka_PTj" %>
    ...
    <div id="pendaftaranTab" class="tabcontent" style="display: block">
        ...
        <div class="row">
            ...
            <div  class="col-md-6">
                ...
                <div style="min-height: 123px;" class=" containerSenaraiSebutHarga "  id="dropTarget"  ></div>
            </div>
         ...
         <div class="draggable row data-template" draggable="true"  id="dragDummy" ondragstart="drag(event)"   style="display:none;">
             <div class="col-4">
               <label >No. Permohonan</label>
...
<script type="text/javascript">
    ...
    async function ajaxSavePerolehan_Mesyuarat_Hdr(Perolehan_Mesyuarat_Header) {
        ...
        return new Promise((resolve, reject) => {
            $.ajax({
                url: '<%= ResolveClientUrl("~/FORMS/PEROLEHAN/PermohonanPoWS.asmx/SavePerolehan_Mesyuarat_Hdr") %>',
                method: 'POST',
    ...
    async function ajaxSavePerolehan_Mesyuarat_Dtl(Perolehan_Mesyuarat_Detail) {
        ...
        return new Promise((resolve, reject) => {
            $.ajax({
                url: '<%= ResolveClientUrl("~/FORMS/PEROLEHAN/PermohonanPoWS.asmx/SavePerolehan_Mesyuarat_Dtl") %>',
            method: 'POST',
    ...
    $(document).ready(async function () {
        ...
        var table = $("#tblDataSenaraiSebutHarga").DataTable({
            "responsive": false,
            "searching": false,
            cache: true,
            dom: 'Bfrtip',
            ...
            "ajax": {
                "url": '<%= ResolveClientUrl("~/FORMS/PEROLEHAN/pembuka.asmx/LoadMesy_PembukaPTJ") %>',
                type: 'POST',
                data: function (d) {
                ...
                "dataSrc": function (json) {
                    ...
                    console.log(parsedData); // Log data to the console
                    return parsedData;
                }
            },
            ...
            "columns": [
                ...
                {
                    "data": null,
                    "defaultContent": '<td style="width: 100%" class="d-flet justify-content-between">' +
                        '<div class="row">' +
                        '<div class="col d-flex justify-content-end align-self-center">' +
                        '<input class="chk_Id" id="chk_Id" value="1" type="checkbox" aria-label="Checkbox for following text input">' +
                        '</div>' +
                        '<div class="col">' +
                        '<input id="sequentData" class="sequence form-control underline-input Debit m-1" minlength="1" maxlength="1" size="1" name="currentSequence" type="text" />' +
                        '</div>' +
                        '</div>' +
                        '</td>',
            ...
            "rowCallback": function (row, data, index) {
                // Set the sequence number when the table is drawn
                $('input.chk_Id', row).on('change', function () {
                    if (this.checked) {
                        var checkedCheckboxes = $(".chk_Id:checked");
                        // Calculate and set the sequence number
                        var sequenceNumber = index + 1;
                        $('input.sequence', row).val(checkedCheckboxes.length);

                        var jsonData = {
                            No_Mohon: data.No_Mohon,
                            Turutan: sequenceNumber
                        }

                        // Check if No_Mohon already exists before pushing
                        if (!isTurutanExists(arrData, jsonData.Turutan)) {
                            arrData.push(jsonData);
                        } else {
                            console.log('Turutan already exists in the array.');
                        }
        ...
        senaraiData.forEach(function (obj) {
            ...
            $template.find(".js-no-permohonan").html(obj.No_Mohon)
        ...
        // Draggable Lable function START
        ...
        function getDragAfterElement(containerSenaraiSebutHarga, y) {
            const draggableElements = [...containerSenaraiSebutHarga.querySelectorAll('.draggable:not(.dragging)')]
        ...
        async function getDataTable() {
            return new Promise((resolve, reject) => {
                $.ajax({
                    url: '<%= ResolveClientUrl("~/FORMS/PEROLEHAN/PermohonanPoWS.asmx/LoadSenarai_Sebut_Harga") %>',
                     type: "POST",
