                                ...
                                <table id="tblDataSenarai_trans" class="table table-striped" style="width: 95%">
                                    <thead>
                                        <tr>
                                            ...
                                            <th scope="col">No. Pengesah</th>
                                            <th scope="col">Nama Pengesah</th>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
            ...
            "columns": [
                ...
                { "data": "AmaunTuntut" },
                {
                    ...
                    render: function (data, type, row, meta) {
                        ...
                        var link = `<button id="btnView1"  data-id = "${data}" class="btn btnView" type="button" data-dismiss="modal" style="color: blue">
                                                    <i class="fa fa-edit"></i>

                                        </button>
                                     <button id="btnView2"  data-id = "${data}" class="btn btnView" type="button" data-dismiss="modal" style="color: blue">
        ...
        $('#tblDataSenarai_trans').on('click', '#btnView2', function () {
          
            var params = `scrollbars=no,resizable=no,status=no,location=no,toolbar=no,menubar=no,width=0,height=0,left=0,top=0`;
            
            var noMohon = $(this).data('id');
          
            window.open('<%=ResolveClientUrl("~/FORMS/EOT/CetakEOT.aspx")%>?bilid=' + noMohon, '_blank', params);
    ...
    $(tblDataSenarai_trans).on('click', '#btnView1', async function () {
        ...
        var no_staf = $(this).closest('tr').find('td:eq(1)').text();
        var no_stafSah = $(this).closest('tr').find('td:eq(3)').text();
        ...
        if (id2 !== "") {
            ...
            let confirm = false
            confirm = await show_message_async("Anda pasti ingin Hantar rekod ini untuk Pengesahan Penyelia ? ")
            console.log(confirm)
            if (!confirm) {
                return
            }
