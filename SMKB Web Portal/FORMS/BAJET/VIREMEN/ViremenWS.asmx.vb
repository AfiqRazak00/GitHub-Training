...
Public Class ViremenWS
    ...
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function SaveOrders_Viremen(order As order_Viremen) As String
        Dim resp As New ResponseRepository
        ...
        If InsertNewOrder(order.OrderID, order.Tujuan, order.Rujukan, order.Jumlah) <> "OK" Then
            ...
            Else
                If InsertNewOrderDtl_K(order.OrderID, order.KW_Drpd, order.KO_Drpd, order.KP_Drpd, order.Ptj_Drpd, order.Vot_Drpd, order.BakiSemasa_Drpd) <> "OK" Then
                    resp.Failed("Gagal Menyimpan Maklumat.")
                    Return JsonConvert.SerializeObject(resp.GetResult())
                End If

                If InsertNewOrderDtl_M(order.OrderID, order.KW_Kpd, order.KO_Kpd, order.KP_Kpd, order.Ptj_Kpd, order.Vot_Kpd, order.BakiSemasa_Kpd) <> "OK" Then
                    resp.Failed("Gagal Menyimpan Maklumat.")
                    Return JsonConvert.SerializeObject(resp.GetResult())
                End If
    ...
    Private Function InsertNewOrder(orderid As String, Tujuan As String, Rujukan As String, Jumlah As String)
        Dim db As New DBKewConn

        Dim queryCheck As String = $"SELECT 1 FROM SMKB_BG_Viremen WHERE No_Viremen = '{orderid}'"
        ...
        If existingRecord Then
            ...
            query = "UPDATE SMKB_BG_Viremen Set Tujuan = @Tujuan, Ruj_Surat = @Ruj_Surat, Kod_Status_Dok = @Kod_Status_Dok, Amaun = @Jumlah, No_Staf = @No_Staf, Tkh_Mohon = '2024-04-23' " &
                   "    WHERE No_Viremen = @No_Viremen "
        Else
            ' Insert a new record
            query = "INSERT INTO SMKB_BG_Viremen (No_Viremen, Tujuan, Ruj_Surat, Kod_Status_Dok, Amaun, No_Staf, Tkh_Mohon) " &
                          " VALUES (@No_Viremen, @Tujuan, @Ruj_Surat, @Kod_Status_Dok, @Jumlah, @No_Staf, GETDATE()) "
        ...
        param.Add(New SqlParameter("@Ruj_Surat", Rujukan))
    ...
    Private Function InsertNewOrder_hh(orderid As String, Tujuan As String, Rujukan As String, Jumlah As String)
        Dim db As New DBKewConn
        Dim query As String = "INSERT INTO SMKB_BG_Viremen (No_Viremen , Tujuan , Ruj_Surat , Kod_Status_Dok, Amaun , No_Staf , Tkh_Mohon)
                              VALUES(@No_Viremen , @Tujuan , @Ruj_Surat , @Kod_Status_Dok, @Jumlah, @No_Staf,  getdate())"
        Dim param As New List(Of SqlParameter)
    ...
    Private Function InsertNewOrderDtl_K(orderid As String, Kod_Kump_Wang As String, Kod_Operasi As String, Kod_Projek As String, Kod_PTJ As String, Kod_Vot As String, bakisms As Decimal)

        Dim db As New DBKewConn
        Dim queryCheck As String = $"SELECT 1 FROM SMKB_BG_ViremenDtl WHERE No_Viremen = '{orderid}' AND Kod_Vir = 'K'"
        Dim existingRecord As Boolean = False
        ...
        If existingRecord Then
            ' Update the existing record
            query = "UPDATE SMKB_BG_ViremenDtl SET Kod_Kump_Wang = @Kod_Kump_Wang, Kod_Operasi = @Kod_Operasi, Kod_Projek = @Kod_Projek, Kod_PTJ = @Kod_PTJ, Kod_Vot = @Kod_Vot, Baki_Sms = @Baki_Sms WHERE No_Viremen = @No_Viremen AND Kod_Vir = 'K'"
        Else
            ' Insert a new record
            query = "INSERT INTO SMKB_BG_ViremenDtl  (No_Viremen, Kod_Kump_Wang, Kod_Operasi, Kod_Projek, Kod_PTJ, Kod_Vot, Kod_Vir, Baki_Sms) " &
                     "VALUES (@No_Viremen, @Kod_Kump_Wang, @Kod_Operasi, @Kod_Projek, @Kod_PTJ, @Kod_Vot, 'K', @Baki_Sms);"
        End If
        ...
        param.Add(New SqlParameter("@Baki_Sms", bakisms))

        Return db.Process(query, param)
    End Function


    Private Function InsertNewOrderDtl_M(orderid As String, Kod_Kump_Wang As String, Kod_Operasi As String, Kod_Projek As String, Kod_PTJ As String, Kod_Vot As String, bakisms As Decimal)

        Dim db As New DBKewConn
        Dim queryCheck As String = $"SELECT 1 FROM SMKB_BG_ViremenDtl WHERE No_Viremen = '{orderid}' AND Kod_Vir = 'M'"
        Dim existingRecord As Boolean = False
        ...
        Else
            ' Insert a new record
            query = "INSERT INTO SMKB_BG_ViremenDtl  (No_Viremen, Kod_Kump_Wang, Kod_Operasi, Kod_Projek, Kod_PTJ, Kod_Vot, Kod_Vir, Baki_Sms) " &
                     "VALUES (@No_Viremen, @Kod_Kump_Wang, @Kod_Operasi, @Kod_Projek, @Kod_PTJ, @Kod_Vot, 'M', @Baki_Sms);"
        End If
    ...
    Private Function GenerateOrderID() As String
        ...
        Dim query As String = $"select TOP 1 No_Akhir as id from SMKB_No_Akhir where Kod_Modul ='01' AND Prefix ='VR' AND Tahun =@year"
        Dim param As New List(Of SqlParameter)
        ...
        If dt.Rows.Count > 0 Then
            ...
            UpdateNoAkhir("01", "VR", year, lastID)
        Else

            InsertNoAkhir("01", "VR", year, lastID)
        End If
        newOrderID = "VR" + Format(lastID, "000000").ToString + month.ToString("00") + Right(year.ToString(), 2)
    ...
    <System.Web.Services.WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function LoadOrderRecord_SenaraiTransaksiViremen(category_filter As String, isClicked As Boolean, tkhMula As DateTime, tkhTamat As DateTime) As String
        ...
        dt = GetOrder_SenaraiTransaksiViremen(category_filter, tkhMula, tkhTamat)
        'resp.SuccessPayload(dt)
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Private Function GetOrder_SenaraiTransaksiViremen(category_filter As String, tkhMula As DateTime, tkhTamat As DateTime) As DataTable
        Dim db = New DBKewConn
        Dim param As List(Of SqlParameter)
        Dim tarikhQuery As String = ""
        ...
        Dim query As String = "select a.No_Viremen, FORMAT (Tkh_Mohon, 'dd-MM-yyyy') as Tkh_Transaksi ,Tujuan, Amaun, No_Staf, Kod_Status_Dok, Ruj_Surat , 
                               virKeluar.kod_kump_wang as kw_K, virKeluar.kod_operasi as ko_K, virKeluar.kod_projek as kp_K, virKeluar.kod_ptj as ptj_K, virKeluar.kod_vot as vot_K , virKeluar.Baki_Sms as bakisms_K,
                                virMasuk.kod_kump_wang as kw_M, virMasuk.kod_operasi as ko_M, virMasuk.kod_projek as kp_M, virMasuk.kod_ptj as ptj_M, virMasuk.kod_vot as vot_M , virKeluar.Baki_Sms as bakisms_M
                                ...
                                AND virMasuk.kod_vir ='M' and Kod_Status_Dok ='01'
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function LoadHdrViremen(ByVal id As String) As String
        Dim resp As New ResponseRepository
    ...
    Private Function GetHdrViremen(id As String) As DataTable
        Dim db = New DBKewConn

        Dim query As String = "select a.No_Viremen, FORMAT (Tkh_Mohon, 'dd-MM-yyyy') as Tkh_Transaksi ,Tujuan, Amaun, No_Staf, Kod_Status_Dok, Ruj_Surat , 
                                concat(virKeluar.kod_kump_wang,' - ',kw.Butiran) as kw_K, 
                                concat(virKeluar.kod_operasi,' - ',ko.Butiran) as ko_K, 
                                concat(virKeluar.Kod_Projek,' - ',kp.Butiran) as kp_K, 
                                concat(virKeluar.kod_ptj,' - ', mj.Pejabat) as ptj_K, 
                                concat(virKeluar.Kod_Vot,' - ',vot.Butiran) as vot_K ,
                                virKeluar.Baki_Sms as bakisms_K ,

                                concat(virMasuk.kod_kump_wang,' - ',kw.Butiran) as kw_M, 
                                concat(virMasuk.kod_operasi,' - ',ko.Butiran) as ko_M, 
                                concat(virMasuk.Kod_Projek,' - ',kp.Butiran) as kp_M, 
                                concat(virMasuk.kod_ptj,' - ', mj.Pejabat) as ptj_M, 
                                concat(virMasuk.Kod_Vot,' - ',vot.Butiran) as vot_M,
                                virMasuk.Baki_Sms as bakisms_M 

                                from smkb_bg_viremen as a 
                                JOIN smkb_bg_viremendtl as virKeluar ON A.NO_Viremen = virKeluar.No_Viremen AND virKeluar.kod_vir ='K'
                                JOIN smkb_bg_viremendtl as virMasuk  ON A.NO_Viremen = virMasuk.No_Viremen AND virMasuk.kod_vir ='M'
                                JOIN SMKB_Kump_Wang AS kw ON virKeluar.Kod_Kump_Wang = kw.Kod_Kump_Wang
                                JOIN SMKB_Operasi AS ko ON virKeluar.Kod_Operasi = ko.Kod_Operasi
                                JOIN SMKB_Projek as kp on virKeluar.Kod_Projek = kp.Kod_Projek
                                JOIN SMKB_Vot AS vot ON virKeluar.Kod_Vot = vot.Kod_Vot
                                JOIN VPejabat AS mj ON mj.kodpejabat =  Left(virKeluar.kod_ptj,2)
                        WHERE a.No_Viremen =@No_Viremen"

        Dim param As New List(Of SqlParameter)
        param.Add(New SqlParameter("@No_Viremen", id))
    ...
    <System.Web.Services.WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function HantarPermohonan_MohonViremen(order As order_Viremen) As String
        Dim resp As New ResponseRepository
        resp.Success("Data telah disimpan")
        Dim success As Integer = 0
        Dim JumRekod As Integer = 0
        ...
        If order.OrderID = "" Then 'untuk permohonan baru
            order.OrderID = GenerateOrderID()
            If InsertNewOrder_hh(order.OrderID, order.Tujuan, order.Rujukan, order.Jumlah) <> "OK" Then
                resp.Failed("Gagal Menyimpan Maklumat.")
                Return JsonConvert.SerializeObject(resp.GetResult())
                ' Exit Function
        ...
        Else
            ...
            If UpdateHantar_MohonViremen(order.OrderID) <> "OK" Then
                resp.Failed("Gagal Menyimpan Maklumat.")
                Return JsonConvert.SerializeObject(resp.GetResult())
            End If
            success = 1
    ...
    Private Function UpdateHantar_MohonViremen(No_Viremen As String)
        Dim db = New DBKewConn
    ...
    <System.Web.Services.WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function HantarPermohonan_MohonViremen_KB(order As order_Viremen) As String
        Dim resp As New ResponseRepository
        resp.Success("Data telah disimpan")
        Dim success As Integer = 0
        Dim JumRekod As Integer = 0
    ...
    Private Function UpdateHantar_MohonViremen_Bend(No_Viremen As String)
        Dim db = New DBKewConn
        Dim query As String = "UPDATE smkb_bg_viremen set Kod_Status_Dok = @Status_Dok ,Tkh_Lulus_Bend = GetDate() where No_Viremen = @No_Viremen"
        Dim param As New List(Of SqlParameter)
    ...
    Private Function UpdateHantar_MohonViremen_NC(No_Viremen As String)
        Dim db = New DBKewConn
        Dim query As String = "UPDATE smkb_bg_viremen set Kod_Status_Dok = @Status_Dok, Tkh_Lulus_NC = GetDate() where No_Viremen = @No_Viremen"
        Dim param As New List(Of SqlParameter)
