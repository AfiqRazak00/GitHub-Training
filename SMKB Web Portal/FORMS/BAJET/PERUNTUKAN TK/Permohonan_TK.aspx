<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/NestedMasterPage2.master" CodeBehind="Permohonan_TK.aspx.vb" Inherits="SMKB_Web_Portal.Permohonan_TK" %>
        ...
        <div class="modal-body">
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <input class="input-group__input" id="txtBakiPeruntukan" type="text" placeholder="&nbsp;" name="Baki Peruntukan" readonly style="background-color: #f0f0f0" />
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            tbl = $("#tblDataSenarai_trans").DataTable({
                ...
                "ajax": {
                    "url": "TambahKurangWS.asmx/LoadOrderRecord_SenaraiTransaksiTK",
                    "method": 'POST',
        ...
        $('.btnSimpan').click(async function () {
            if ($('#ddlDasar').val() === null) {
                $('#maklumanModalBil').modal('toggle');
                $('#detailMaklumanBil').html("Sila pilih maklumat Dasar");
                return;
            }
        ...
        async function ajaxSaveOrder_Hantar(order) {
            ...
            return new Promise((resolve, reject) => {
                $.ajax({
                    url: 'TambahKurangWS.asmx/HantarPermohonan_MohonTK',
                    method: 'POST',
                    data: JSON.stringify(order),
                    dataType: 'json',
                    contentType: 'application/json; charset=utf-8',
        ...
        async function setValueToRow_HdrBajet(orderDetail) {
            ...
            //ddlDasar
            var selectObj_ddlDasar = $('#ddlDasar')
            $(ddlDasar).dropdown('set selected', orderDetail.dasar);
            selectObj_ddlDasar.append("<option value = '" + orderDetail.dasar + "'>" + orderDetail.dasar_Butir + "</option>")
            //ddlKatPeruntukan
            var selectObj_ddlKatPeruntukan = $('#ddlDasar')
            $(ddlKatPeruntukan).dropdown('set selected', orderDetail.Kod_Agih);
            selectObj_ddlKatPeruntukan.append("<option value = '" + orderDetail.Kod_Agih + "'>" + orderDetail.Butir_Agih + "</option>")
            $('#txtNoMohon').val(orderDetail.No_Mohon)
            $('#txtTahun').val(orderDetail.Tahun_Bajet)
            $('#txtButiran').val(orderDetail.Justifikasi)
            // Assuming orderDetail.Jumlah_Mohon is in numeric format
            var amount = parseFloat(orderDetail.Jumlah_Mohon);
            var formattedAmount = amount.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
            $('#txtAmaun').val(formattedAmount);
            ...
            today = yyyy + '-' + mm + '-' + dd;
            var BakiBajet = getBakiBajet(yyyy, today, orderDetail.kw, orderDetail.ko, orderDetail.ptj, orderDetail.kp, orderDetail.vot)

            var koddasar =  orderDetail.dasar

            await initDropdownDasar(koddasar)
        ...
        $('#ddlCOA').dropdown({
            ...
            apiSettings: {
                //url: '<%= ResolveUrl("~/FORMS/BAJET/PERUNTUKAN TK/TambahKurang_WS.asmx/GetVotCOA") %>' + '?q={query}',
        ...
        $('#ddlCOA_Kpd').on('click', function () {
            ...
            if ($('#txtKW_Drpd').val() === null || $('#txtKW_Drpd').val() === '') {
                ...
                console.log('Sila Pilih KW kepada untuk buat permohonan');
