...
Public Class Transaksi_InvoisWS
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function GetPemiutangListByCategory(ByVal q As String, ByVal category As String) As String
        Dim tmpDT As DataTable = GetPenerimaList(q, category)
        Return JsonConvert.SerializeObject(tmpDT)
    End Function
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Private Function GetPenerimaList(kod As String, category As String) As DataTable
        ...
        Dim param As New List(Of SqlParameter)
        query &= " AND (Kod_Pemiutang LIKE '%' + @kod + '%' OR Nama_Pemiutang LIKE '%' + @kod2 + '%') AND Kategori_Pemiutang LIKE '%' + @Kategori + '%'"
        param.Add(New SqlParameter("@kod", kod))
        param.Add(New SqlParameter("@kod2", kod))
        param.Add(New SqlParameter("@Kategori", category))
