...
Public Class Baucar_WS
    ...
    'senarai Kelulusan yg belum ada atau masih draf baucar
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function getKelulusan(ByVal No_Baucar As String) As String
        ...
        Try
            ...
            stat.Add("11")
            stat.Add("12")
            res.Payload = SharedModulePembayaran.getDOKLog(No_Baucar, stat)
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function getIntegrationDrafBaucar(ByVal DateStart As String, ByVal DateEnd As String) As String
        Dim req As Response = fetchIntegrationForDrafBaucar(DateStart, DateEnd)
        Return JsonConvert.SerializeObject(req)
    End Function

    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function getIntegrationDrafBaucar_Pinjaman(ByVal DateStart As String, ByVal DateEnd As String) As String
        Dim req As Response = fetchIntegrationForDrafBaucar_Pinjaman(DateStart, DateEnd)
        Return JsonConvert.SerializeObject(req)
    End Function

    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function getIntegrationDrafBaucar_Tuntutan(ByVal DateStart As String, ByVal DateEnd As String) As String
        Dim req As Response = fetchIntegrationForDrafBaucar_Tuntutan(DateStart, DateEnd)
        Return JsonConvert.SerializeObject(req)
    End Function

    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function getIntegrationDrafBaucar_Pendahuluan(ByVal DateStart As String, ByVal DateEnd As String) As String
        Dim req As Response = fetchIntegrationForDrafBaucar_Pendahuluan(DateStart, DateEnd)
        Return JsonConvert.SerializeObject(req)
    End Function

    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function getIntegrationDrafBaucar_Pelajar(ByVal DateStart As String, ByVal DateEnd As String) As String
        Dim req As Response = fetchIntegrationForDrafBaucar_Pelajar(DateStart, DateEnd)
        Return JsonConvert.SerializeObject(req)
    End Function
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function getBaucarDtl_Integrasi(No_Baucar As String) As String
        Dim sqlcmd As New SqlCommand
        Dim dt As New DataTable
        Try
            Using sqlconn As New SqlConnection(dbSMKB.strCon)
                sqlconn.Open()
                sqlcmd.Connection = sqlconn
                sqlcmd.CommandText = "Select No_Order AS ID_Rujukan,iv.Kod_Vot,iv.Kod_PTJ,iv.Kod_Kump_Wang,iv.Kod_Operasi,iv.Kod_Projek,iv.No_Dokumen AS Kod_Pemiutang, iv.Jumlah AS Kadar_Harga,'0' AS Cukai,'1' AS Kuantiti_Sebenar,'0' AS Diskaun,
                    dtl.No_Baucar,dtl.No_Item,Jumlah_Bayar,dtl.Kod_Cukai,dtl.Cara_Bayar, pm.Kod_Pemiutang, pm.Nama_Pemiutang
                    FROM SMKB_Pembayaran_Baucar_Dtl dtl 
                    JOIN SMKB_Pembayaran_Baucar_Hdr hdr ON hdr.No_Baucar = dtl.No_Baucar 
                    JOIN AP_Order_Dtl iv ON iv.No_Order = hdr.ID_Rujukan AND iv.No_Item = dtl.No_Item
                    JOIN SMKB_Pemiutang_Master pm ON pm.Kod_Pemiutang = iv.No_Dokumen
                    WHERE hdr.No_Baucar =@No_Baucar"
                sqlcmd.Parameters.Add(New SqlParameter("@No_Baucar", No_Baucar))
                dt.Load(sqlcmd.ExecuteReader())
            End Using
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=True, ResponseFormat:=ResponseFormat.Json)>
    Public Function getBaucarByIDRujukan_Integrasi(ID_Rujukan As String) As String
        Dim response As New Response
        Try
            Using sqlconn As New SqlConnection(dbSMKB.strCon)
                Dim cmd As SqlCommand
                ...
                Else
                    ...
                    If Not cmdInsert.ExecuteNonQuery() = 0 Then
                        ...
                        cmdDtl.CommandText = "INSERT INTO SMKB_Pembayaran_Baucar_Dtl (No_Baucar , No_Item)
                                                    SELECT @No_Baucar,No_Item
                                                    FROM AP_Order_Dtl WHERE No_Order = @ID_Rujukan"
                        cmdDtl.Parameters.Add(New SqlParameter("@No_Baucar", noBaucar))
                        cmdDtl.Parameters.Add(New SqlParameter("@ID_Rujukan", ID_Rujukan))
    ...
    Private Function fetchInvoisSumByBaucarDok(Status_Dok As List(Of String), DateStart As String, DateEnd As String) As Response
        ...
        Try
            Using sqlconn As New SqlConnection(dbSMKB.strCon)
				...
				LEFT JOIN SMKB_Kod_Status_Dok kdsts ON bhdr.Status_Dok=kdsts.Kod_Status_Dok AND Kod_Modul='03'
                WHERE bhdr.Status_Dok IN (" & statusList & ") "

                If Status_Dok.Contains("11") Then
                    sqlText += " AND dtl.Jumlah > @Jumlah "
                    sqlcmd.Parameters.Add(New SqlParameter("@Jumlah", 10000))
                End If
    ...
    Private Function fetchIntegrationForDrafBaucar(DateStart As String, DateEnd As String) As Response
        Dim sqlcmd As New SqlCommand
        Dim dt As New DataTable
        Dim res As New Response
        res.Code = 200
        Try
            Using sqlconn As New SqlConnection(dbSMKB.strCon)
                ...
                Dim sqlText As String = "Select hdr.No_Order AS ID_Rujukan,hdr.ID_Rujukan AS No_Rujukan,'' AS No_Invois, hdr.Tkh_Transaksi AS Tarikh_Daftar, Tkh_Transaksi AS Tarikh_Invois,Tkh_Transaksi AS Tarikh_Terima_Invois, hdr.Pengirim_Mohon AS Tujuan,
                jb.Kod + ' - ' + jb.Butiran as Jenis_Bayar,
                ji.Kod + ' - ' + ji.Butiran as Jenis_Invois,ji.Kod,
                dtl.Jumlah,
				(SELECT SUBSTRING((
				SELECT distinct ' , ' + Nama_Pemiutang 
				FROM AP_Order_Hdr A,
				AP_Order_Dtl B,
				SMKB_Pemiutang_Master C  
				WHERE A.No_Order=B.No_Order AND B.No_Dokumen=C.Kod_Pemiutang
				AND A.No_Order=hdr.No_Order  FOR XML PATH('')
				), 3, 1000) AS Combined_Names) AS Nama_Pemiutang,kdsts.Butiran AS Status
                From AP_Order_Hdr hdr
                JOIN 
                SMKB_Pembayaran_Jenis_Bayar jb ON 'BP' = jb.Kod
                JOIN
                SMKB_Pembayaran_Jenis_Invois ji ON 'J03' = ji.Kod
                LEFT JOIN
                SMKB_Pembayaran_Baucar_Hdr bhdr ON hdr.No_Order = bhdr.ID_Rujukan AND hdr.Pengirim='GAJI'
                LEFT JOIN
                (SELECT
                    No_Order,
                    SUM(
                        COALESCE(Jumlah, 0) * 
                        COALESCE(1, 0)
						-(COALESCE(Jumlah, 0) *  COALESCE(1, 0) * COALESCE(0, 0) / 100) 
                        +(COALESCE(Jumlah, 0) *  COALESCE(1, 0) * COALESCE(0, 0) / 100)
						 
                    ) AS Jumlah, '08' AS Status
                FROM
                    AP_Order_Dtl
                GROUP BY
                    No_Order
                ) dtl ON hdr.No_Order = dtl.No_Order 
				LEFT JOIN SMKB_Kod_Status_Dok kdsts ON Status_Dok=kdsts.Kod_Status_Dok AND Kod_Modul='03'
                WHERE (bhdr.Status_Dok IS NULL OR bhdr.Status_Dok='08') AND hdr.Pengirim='GAJI'"



                If DateStart IsNot Nothing And DateStart <> "" Then
                    sqlText += " AND hdr.Tkh_Transaksi >= @DateStart "
                    sqlcmd.Parameters.Add(New SqlParameter("@DateStart", DateStart))
                End If

                If DateEnd IsNot Nothing And DateEnd <> "" Then
                    sqlText += " AND hdr.Tkh_Transaksi <= @DateEnd "
                    sqlcmd.Parameters.Add(New SqlParameter("@DateEnd", DateEnd))
                End If
    ...
    Private Function fetchIntegrationForDrafBaucar_Pinjaman(DateStart As String, DateEnd As String) As Response
        Dim sqlcmd As New SqlCommand
        Dim dt As New DataTable
        Dim res As New Response
        res.Code = 200
        Try
            Using sqlconn As New SqlConnection(dbSMKB.strCon)
                ...
                SMKB_Pembayaran_Jenis_Bayar jb ON 'BP' = jb.Kod
                JOIN
                SMKB_Pembayaran_Jenis_Invois ji ON 'J04' = ji.Kod
                LEFT JOIN
                SMKB_Pembayaran_Baucar_Hdr bhdr ON hdr.No_Order = bhdr.ID_Rujukan AND hdr.Pengirim='PINJ'
				LEFT JOIN
                (SELECT
                    No_Order,
                    SUM(
				...
				LEFT JOIN SMKB_Kod_Status_Dok kdsts ON Status_Dok=kdsts.Kod_Status_Dok AND Kod_Modul='03'
                WHERE (bhdr.Status_Dok IS NULL OR bhdr.Status_Dok='08') AND hdr.Pengirim='PINJ'"
    ...
    Private Function fetchIntegrationForDrafBaucar_Tuntutan(DateStart As String, DateEnd As String) As Response
        Dim sqlcmd As New SqlCommand
        Dim dt As New DataTable
        Dim res As New Response
        res.Code = 200
        Try
            Using sqlconn As New SqlConnection(dbSMKB.strCon)
                ...
                SMKB_Pembayaran_Jenis_Bayar jb ON 'BP' = jb.Kod
                JOIN
                SMKB_Pembayaran_Jenis_Invois ji ON 'J07' = ji.Kod
                LEFT JOIN
                SMKB_Pembayaran_Baucar_Hdr bhdr ON hdr.No_Order = bhdr.ID_Rujukan AND hdr.Pengirim='CLAIM'
				...
				LEFT JOIN SMKB_Kod_Status_Dok kdsts ON Status_Dok=kdsts.Kod_Status_Dok AND Kod_Modul='03'
                WHERE (bhdr.Status_Dok IS NULL OR bhdr.Status_Dok='08') AND hdr.Pengirim='CLAIM'"
    ...
    Private Function fetchIntegrationForDrafBaucar_Pendahuluan(DateStart As String, DateEnd As String) As Response
        Dim sqlcmd As New SqlCommand
        Dim dt As New DataTable
        Dim res As New Response
        res.Code = 200
        Try
            Using sqlconn As New SqlConnection(dbSMKB.strCon)
                ...
                SMKB_Pembayaran_Jenis_Bayar jb ON 'BP' = jb.Kod
                JOIN
                SMKB_Pembayaran_Jenis_Invois ji ON 'J06' = ji.Kod
                LEFT JOIN
                SMKB_Pembayaran_Baucar_Hdr bhdr ON hdr.No_Order = bhdr.ID_Rujukan AND hdr.Pengirim='ADV'
                LEFT JOIN
                (SELECT
                    No_Order,
                    SUM(
				...
				LEFT JOIN SMKB_Kod_Status_Dok kdsts ON Status_Dok=kdsts.Kod_Status_Dok AND Kod_Modul='03'
                WHERE (bhdr.Status_Dok IS NULL OR bhdr.Status_Dok='08') AND hdr.Pengirim='ADV'"
    ...
    End Function
    Private Function fetchIntegrationForDrafBaucar_Pelajar(DateStart As String, DateEnd As String) As Response
        Dim sqlcmd As New SqlCommand
        Dim dt As New DataTable
        Dim res As New Response
        res.Code = 200
        Try
            Using sqlconn As New SqlConnection(dbSMKB.strCon)
                ...
                SMKB_Pembayaran_Jenis_Bayar jb ON 'BP' = jb.Kod
                JOIN
                SMKB_Pembayaran_Jenis_Invois ji ON 'J12' = ji.Kod
                LEFT JOIN
                SMKB_Pembayaran_Baucar_Hdr bhdr ON hdr.No_Order = bhdr.ID_Rujukan AND hdr.Pengirim IN ('SMP','SMPS','SMPSH')
                LEFT JOIN
                (SELECT
                    No_Order,
                    SUM(
				...
				LEFT JOIN SMKB_Kod_Status_Dok kdsts ON Status_Dok=kdsts.Kod_Status_Dok AND Kod_Modul='03'
                WHERE (bhdr.Status_Dok IS NULL OR bhdr.Status_Dok='08') AND hdr.Pengirim IN ('SMP','SMPS','SMPSH')"
