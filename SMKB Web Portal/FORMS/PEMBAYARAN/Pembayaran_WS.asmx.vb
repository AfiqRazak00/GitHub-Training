...
Public Class SharedPembayaran_WS
    ...
    Public Function getKategoriPemiutang(ByVal q As String) As String
        Dim response As New Response
        Dim dt As DataTable
        If q.IsNullOrWhiteSpace Then
            dt = findInLookupDetails("0152")
        Else
            dt = findInLookupDetails("0152", q)
        End If
        Try
            response.Payload = dt
            response.Code = 200
...
Module SharedModulePembayaran
    ...
    Public Function getDOKLog(No_Rujukan As String, Optional Status As List(Of String) = Nothing) As DataTable
        ...
        Using sqlconn As New SqlConnection(dbSMKB.strCon)
            ...
            Dim sql As String = "SELECT p.*,d.Kod_Status_Dok,d.Tarikh FROM  
                SMKB_Pemiutang_Master p JOIN (  
                    SELECT  Kod_Status_Dok,No_Staf, MAX(Tkh_Tindakan) as Tarikh FROM SMKB_Status_Dok 
                    WHERE No_Rujukan = @No_Rujukan "
            If Status IsNot Nothing Then
                Dim statusList As String = String.Join(",", Status.Select(Function(s) $"'{s}'"))
                sql += " AND Kod_Status_Dok IN (" + statusList + ") "
            End If
            sql += "GROUP BY Kod_Status_Dok,No_Staf) d 
                ON d.No_Staf = p.No_Rujukan"
