...
Public Class BukuVot
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            Dim VotDr As DataTable = GetVotDari()
            txtVotDari.DataSource = VotDr
            txtVotDari.DataBind()

            Dim VotHg As DataTable = GetVotHingga()
            txtVotHingga.DataSource = VotHg
            txtVotHingga.DataBind()
    ...
    Private Function GetVotDari() As DataTable

        Dim connection As New SqlConnection(strCon)
        Dim query As String = " select Kod_Vot , Concat(Kod_Vot,' - ',Butiran) as Butiran from SMKB_Vot WHERE Kod_Klasifikasi ='h2' and status = 1 ORDER BY Kod_Vot"

        Dim command As New SqlCommand(query, connection)
        Dim VotDr As New DataTable()
        connection.Open()
    ...
    Private Function GetVotHingga() As DataTable

        Dim connection As New SqlConnection(strCon)
        Dim query As String = " select Kod_Vot ,  Concat(Kod_Vot,' - ',Butiran) as Butiran from SMKB_Vot WHERE Kod_Klasifikasi ='h2' and status = 1 ORDER BY Kod_Vot"

        Dim command As New SqlCommand(query, connection)
        Dim VotHg As New DataTable()
        connection.Open()
    ...
    Private Function GetPtjList() As DataTable
        ...
        select concat(KodPejabat,'0000') as KodPejabat , Pejabat from VPejabat where Status = 1"
