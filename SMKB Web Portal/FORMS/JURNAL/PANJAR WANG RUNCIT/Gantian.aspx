                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <div class="form-row">
                                    <div class="form-group col-md-4">
                                        <input type="text" class="input-group__input" id="lblJumlahTambah" name="lblJumlahTambah" placeholder="&nbsp;" readonly style="background-color: #f0f0f0" value="0.00"/>
                                        <label class="input-group__label" for="Jumlah Diterima">Jumlah Diterima</label>
                                    </div>
                                    <div class="form-group col-md-4">
                                        <input type="text" class="input-group__input" id="lblJumlahMohon" name="Jumlah Mohon" placeholder="&nbsp;" readonly style="background-color: #f0f0f0" value="0.00" hidden/>
                                        <label class="input-group__label" for="Jumlah Mohon" hidden>Jumlah Mohon</label>
                                    </div>
                                    <div class="form-group col-md-4">
                                        <input type="text" class="input-group__input" id="lblJumlahTolak" name="lblJumlahTolak" placeholder="&nbsp;" readonly style="background-color: #f0f0f0" value="0.00" hidden/>
                                        <label class="input-group__label" for="Jumlah Tolak" hidden>Jumlah Tolak</label>
                                    </div>
                            ...
                            <div class="col-md-12">
                                <div class="application-table table-responsive">
                                    <table class="table table-striped" id="tblPemeriksaan">
                                        <thead>
                                            <tr>
                                                    <th scope="col" style="text-align: center;">Gantian</th>
                                        ...
                                        <tbody class="tableID">
                                            <tr style="display: none; width: 100%" class="table-list">
                                                 <td class="auto-style1" style="text-align: center;">
                                                    <input type="checkbox" id="chkResit" name="chkResit" class="chkResit" >
                                                    <label for="chkResit" class="label-chck"></label>
                                                    <label id="HidlblCheck" name="HidlblCheck" class="HidlblCheck" style="visibility: hidden"></label>
                                                      <label id="Hid-Bil" name="Hid-Bil" class="Hid-Bil" style="visibility: hidden"></label>
                                                </td>
...
<%--                                                <td class="auto-style1">
                                                    <input id="paparJumlahMohon" name="paparJumlahMohon" runat="server" type="number" class="form-control paparJumlahMohon"
        ...
        <script>
            ...
            function toggleCheckboxes(checkbox) {
                const checkboxes = document.querySelectorAll('.checkbox-resit');
            ...
            function calculate(checkbox) {
                ...
                var table = $('#tblPemeriksaan').DataTable();
                var row = $(checkbox).closest('tr');
                var rowIndex = table.row(row).index();
                // Get the data for the row
                var rowData = table.row(row).data();
                // Access the 'jumlah' column in the rowData
                var jumlahMohon = $(row).find('.Mohon').val();

                if (isChecked) {
                   
                    //kalau untick,, tambah value dia jumlahtambah, tolak dalam jumlahtolak

                    var lblJumlahTambahValue = $('#lblJumlahTambah').val();
                    var lblJumlahTolakValue = $('#lblJumlahTolak').val();

                    var totalAdd = parseFloat(lblJumlahTambahValue) + parseFloat(jumlahMohon)
                    $('#lblJumlahTambah').val(totalAdd);

                    var totalminus = parseFloat(lblJumlahTolakValue) - parseFloat(jumlahMohon)
                    $('#lblJumlahTolak').val(totalminus);
                    // Find all elements with class "jumlah" within the checked row
                    //var jumlahTotal = 0;
                    //$(row).find('.Mohon').each(function () {
                    //    jumlahTotal += parseFloat($(this).val());
                    //    alert(jumlahTotal)
                    //});
                    //alert("column: " + jumlahTotal);                   

                }

                else {
                    
                    //kalau untick,, tolak value dia jumlahtambah, masuk dalam jumlahtolak
                    var lblJumlahTambahValue = $('#lblJumlahTambah').val();
                    var lblJumlahTolakValue = $('#lblJumlahTolak').val();
            ...
            $(document).ready(function () {
                ...
                tbl = $('#tblDataSenarai_perm').DataTable({
                    ...
                    ajax: {
                        url: "Panjar_WS.asmx/LoadOrderRecord_SenaraiPemeriksaan_Kelulusan_gantian",
                        method: 'POST',
            ...
            $('.btnLulus').off('click').on('click', async function () {
                ...
                var newOrderM = {
                    orderM: {
                        ...
                        BakiSemasa: $('#lblBakiSemasa').val(),
                        JumlahTambah: $('#lblJumlahTambah').val(),
                        JumlahTolak: $('#lblJumlahTolak').val(),
                        OrderMDetails: []
                    }
                ...
                $('.chkResit').each(function (index, obj) {

                    if (index > 0) {
                        var tcell = $(obj).closest("td");

                        var check;
                        //$('.chkResit').each(function () {
                            var checkboxIndex = $(this).index('.chkResit');

                            if ($(this).is(':checked')) {
                                check = 1;
                            } else {
                                // If checkbox is not checked, set check to something else
                                check = 0;
                            }
                        ...
                        var orderMDetail = {
                            ...
                            OrderMId: $('#lblNoSiri').val(),
                            /* checkBox: $('#chkResit').is(':checked'),*/
                            Bil: $('.Hid-Bil').eq(index).html(),
                            checkBox: check,
                            ...
                            tarikh_resit: $('.lblTkhResit').eq(index).val(),
                            baki: $('.Baki').eq(index).val(),
                            id: $(tcell).find(".data-id").val()
                        };
                        console.log(orderMDetail)
                        //console.log($('.lblTkhResit').eq(index).val())
                        acceptedRecord += 1;
                ...
                msg = "Anda pasti untuk simpan permohonan ini";
                ...
                $('#confirmButton').off('click').on('click', async function () {
                    ...
                    var result = JSON.parse(await ajaxSaveOrderSemak(newOrderM));
            ...
            async function setValueToRow_HdrPemeriksaan(orderMDetail) {
                ...
                $('#lblJumlahTambah').val(orderMDetail.Jumlah_Belanja);
                $('#lblJumlahTolak').val(orderMDetail.Jumlah_Tolak);
            ...
            async function setValueToRow_Pemeriksaan(row, orderMDetail) {
                ...
                var colBilHid = $(row).find("td > .Hid-Bil");
                colBilHid.html(orderMDetail.Bil);

                //var chkResit = $(row).find("td > .chkResit");               
                ////alert("xx " + orderMDetail.Status)
                //// Check the checkbox if orderMDetail.Status is "L"
                //var statusCheck  = orderMDetail.Status
                ////alert("xx " + statusCheck)
                ////console.log("Status Check:", statusCheck);

                //if (statusCheck === "L") {                   
                //    chkResit.prop('checked', true);                              
                //}
                //else {
                //    chkResit.prop('checked', false);
                //}
                ...
                var tkh_resit = $(row).find("td > .lblTkhResit");
                tkh_resit.val(orderMDetail.Tarikh_Resit);

                var mohon = $(row).find("td > .Mohon");
                mohon.val(orderMDetail.Jumlah_Butiran);

                var jumlah = $(row).find("td > .paparJumlahMohon");
                jumlah.val(orderMDetail.Jumlah_Butiran);

                var baki = $(row).find("td > .Baki");
                baki.val(orderMDetail.Baki_Peruntukan);
            ...
            async function calculateGrandTotal() {
                ...
                var curTotal_Tolak = 0;

                $('.Mohon').each(function (index, obj) {
                    curTotal_Jumlah += parseFloat(NumDefault($(obj).val()));
                    curTotal_Tolak += parseFloat(NumDefault($(obj).val()));
                });
                totalJumlah.val(curTotal_Jumlah.toFixed(2));
                //$('#lblJumlahTolak').val(curTotal_Tolak.toFixed(2));
                $('#lblJumlahMohon').val(curTotal_Tolak.toFixed(2));
            ...
            async function AjaxGetRecordPermohonan(id) {
                ...
                try {
                    ...
                    const response = await fetch('Panjar_WS.asmx/LoadRecordPermohonan_gtn', {
                        method: 'POST',
                        headers: {
            ...
            //async function ajaxSaveOrderSemak(newOrderM) {
            //alert("a")
            //    return new Promise((resolve, reject) => {
            //        $.ajax({
            //            url: 'Panjar_WS.asmx/SemakOrderSemak',
            //            method: 'POST',
            ...
            async function ajaxSaveOrderSemak(orderM) {

                return new Promise((resolve, reject) => {
                    $.ajax({
                        url: 'Panjar_WS.asmx/SemakOrderLulus_ganti',
                        method: 'POST',
                        data: JSON.stringify(orderM),
                        dataType: 'json',
                        contentType: 'application/json; charset=utf-8',
