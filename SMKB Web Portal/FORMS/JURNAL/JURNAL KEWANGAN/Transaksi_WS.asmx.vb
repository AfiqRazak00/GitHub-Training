...
Public Class Transaksi_WS
    ...
    <System.Web.Services.WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function SaveOrders_Dtl(order As Order) As String
        Dim resp As New ResponseRepository
        resp.Success("Data telah disimpan")
        Dim success As Integer = 0
        Dim JumRekod As Integer = 0
    ...
    'Private Async Function UpdateLulusOrderDetail(orderid As String) As Tasks.Task
    '    Dim db As New DBKewConn
    ...
    Private Function GetOrder_SenaraiLulusTransaksiJurnal(category_filter As String, tkhMula As DateTime, tkhTamat As DateTime) As DataTable
		...
		Left join VPeribadi as c on c.MS01_NoStaf = b.No_Staf
    ...
    <System.Web.Services.WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Async Function Lulusorder(order As Order) As Tasks.Task(Of String)
        Dim resp As New ResponseRepository
        ...
        Else
            ...
            For Each orderDetail As OrderDetail In order.OrderDetails
                Try
                    ...
                    Dim values As String = Await servicex.SendDataLejar(myGetTicket.GetTicket("smkb", Session("ssusrID")),
                                        "GL", "UTeM", kodkw, kodptj,
                                        kodvot, kodko, kodkp, orderDetail.debit, "DR", vBulan, vTahun)
    ...
    Public Async Function EmelStaf(orderID As String) As Tasks.Task(Of String)
        ...
        Dim fullName_Penerima As String = "Syafiqah Binti Abdul Jalil"
        Dim email_Penerima As String = "syafiqah@utem.edu.my"
        Dim NoStaff_Penerima = "02634"
        Dim TarikhLuput = "2024-04-29" 'kenalpasti tarikh active

        Dim KodSubMenu = "040103" 'identify kod sub menu skrin sokong/lulus ikut modul msing2
        ...
        If db.fInsertCommand(strSqlButiran, paramSqlBtrn) > 0 Then
            ...
            Dim url As String = "~/loginsmkb.aspx?id=" & id 'ResolveUrl("~/loginsmkb.aspx?id=" & id) 
            ...
            Dim body As String = "Kelulusan Transaksi Jurnal " &
                         ...
                         "<br><br>" &
                         vbCrLf & "Sila klik di link ini untuk menyemak transaksi jurnal untuk kelulusan melalui <a href=" + url + "></a>" &
                         "<br>" &
                         "<br><br>" &
            ...
            ''Notification API
            ' API endpoint URL
            Dim apiUrl As String = "https://devmobile.utem.edu.my/smkbnotification/api/notification/smkb/SISTEM MAKLUMAT KEWANGAN BERSEPADU/Kelulusan Transaksi Jurnal.Memerlukan kelulusan anda untuk transaksi jurnal./" + id + "/" + NoStaff_Penerima
    ...
    'Private Async Function NotifyApps(id As String, NoStaff_Penerima As String) As Tasks.Task
    '    ' API endpoint URL
    '    Dim apiUrl As String = "https://devmobile.utem.edu.my/smkbnotification/api/notification/smkb/SISTEM MAKLUMAT KEWANGAN (KELULUSAN PERMOHONAN JURNAL)/Memerlukan kelulusan anda untuk permohonan jurnal. Sila klik di link ini untuk menyemak transaksi jurnal melalui /" + id + "/" + NoStaff_Penerima
