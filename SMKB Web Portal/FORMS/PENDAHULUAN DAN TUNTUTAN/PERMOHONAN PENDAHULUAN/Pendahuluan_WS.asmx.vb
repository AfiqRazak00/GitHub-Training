...
Public Class Pendahuluan_WS
    ...
    Private Function GetKodCOAList(kodCariVot As String) As DataTable
        ...
        Dim query As String = "
                                SELECT CONCAT(a.Kod_Kump_Wang, ' - ', REPLACE(kw.Butiran, 'KUMPULAN WANG', 'KW'), ' , ' , mj.Pejabat, ', ', a.Kod_Vot, ' -', vot.Butiran,  ' , ', ko.Butiran, ', ' ,kp.Butiran) AS text,
                                ...
                                JOIN SMKB_Lookup_Detail AS lookDtl ON lookDtl.Kod = 'AC15' and a.Kod_Vot = lookDtl.Butiran
    ...
    Private Function GetKodPtjList2(kod As String) As DataTable
        Dim db As New DBKewConn
        Dim query As String = "
                                Select distinct Kod_PTJ as value, 
        ...
        If kod <> "" Then
            query &= " where Status = 1 and (Kod_PTJ LIKE '%' + @kod + '%' or 
                    ...
                    WHERE b.kodpejabat = left(Kod_PTJ,2) and b.Pejabat like '%' + @kod2 + '%'))) order by Kod_PTJ"
            param.Add(New SqlParameter("@kod", kod))
            param.Add(New SqlParameter("@kod2", kod))
        End If
    ...
    Private Function GetRecord_SenaraiSendiri(category_filter As String, tkhMula As DateTime, tkhTamat As DateTime, staffP As String) As DataTable
        ...
        Dim query As String = "SELECT a.No_Pendahuluan, a.Tarikh_Mohon,  FORMAT(a.Tarikh_Mohon,'dd-MM-yyyy') as Tarikh_MohonDisplay, a.No_Staf + ' - ' + c.ms01_nama   as NamaPemohon, 
                    ...
                    a.Ptj, (SELECT b.Pejabat FROM VPejabat as b WHERE b.kodpejabat = left(Ptj,2)) as ButiranPTJ ,
    ...
    Private Function GetHdrPermohonan(id As String) As DataTable
        ...
        Dim query As String = "SELECT DISTINCT a.No_Pendahuluan, FORMAT(a.Tarikh_Mohon, 'yyyy-MM-dd') AS Tarikh_Mohon, a.Status, a.Jenis_Pendahuluan, a.Tujuan, a.Jum_Mohon, b.Butiran, b.Kod_Modul, b.Kod_Status_Dok, 
                        ...
                        a.Kod_PTJ,(SELECT  a.Kod_PTJ FROM   VPejabat AS b WHERE (a.Kod_PTJ = LEFT(a.PTj, 2))) AS ButiranPTJ, 
    ...
    Private Function GetDataPermohonan(kod As String) As DataTable
        ...
        Dim query As String = "SELECT a.No_Pendahuluan, a.Tujuan, a.Tempat_Perjalanan, a.Tarikh_Mula, a.Tarikh_Tamat, a.Tempoh_Pjln, a.Status, a.JenisTempat,a.Tkh_Adv_Perlu,
                        ...
                        a.Jenis_Penginapan, (SELECT  Butiran  FROM  SMKB_Lookup_Detail as jpn WHERE jpn.Kod='AC01'  AND (a.Jenis_Penginapan = jpn.Kod_Detail)) AS ButiranJenisPginap,
                        a.CaraBayar, (SELECT  Butiran  FROM  SMKB_Lookup_Detail as jb WHERE jb.Kod='0018'  AND (a.CaraBayar = jb.Kod_Detail)) AS ButiranJenisBayar,
                        Kod_Kump_Wang as colhidkw ,(select Butiran from SMKB_Kump_Wang as kw where a.Kod_Kump_Wang = kw.Kod_Kump_Wang) as colKW,
                        '00' as colhidko, (select Butiran from SMKB_Operasi as ko where  ko.Kod_Operasi = '00') as colKO,
                        '0000000' as colhidkp,  (select Butiran from SMKB_Projek as kp where kp.Kod_Projek = '0000000') as colKp,
                        Ptj as colhidptj , (SELECT b.Pejabat FROM VPejabat as b WHERE b.kodpejabat = left(Ptj,2)) as ButiranPTJ ,
    ...
    Public Function SaveRecordAdv(AdvList As MhnAdvance) As String
        ...
        If AdvList.OrderID = "" Then 'untuk permohonan baru
            ...
            'AdvList.Folder = "UPLOAD/PENDAHULUAN DAN TUNTUTAN/PD/"
    ...
    Private Function InsertNewOrder(AdvList As MhnAdvance)
        ...
        Dim query As String = "INSERT INTO SMKB_Pendahuluan_Hdr(No_Pendahuluan, No_Staf, Samb_Telefon, PTj, Kategori, Jenis_Pendahuluan, Tujuan, 
                                ...
                                Jenis_Penginapan, If_Mkn, Tkh_Adv_Perlu, Status_Dok, Status, CaraBayar, Tarikh_Mohon, Sbb_Lewat)
        ...
        param.Add(New SqlParameter("@justLewat", AdvList.JustLewat)) ' - faiz
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Private Function LoadUploadDokumenDtl(noPendahuluan As String) As DataTable
        Dim db = New DBKewConn
        ...
        Using sqlconn As New SqlConnection(connectionString)
            ...
            query = "
                 Select * From SMKB_Pendahuluan_Dok_Upload Where No_Pendahuluan = @nopdhuluan
            "
            cmd.Connection = sqlconn
            cmd.CommandText = query

            cmd.Parameters.Add(New SqlParameter("@nopdhuluan", noPendahuluan))
    ...
    'faiz
    <WebMethod(EnableSession:=True)>
    Function AddDokumen() As String
        Dim postedFile As HttpPostedFile = HttpContext.Current.Request.Files(0)
        Dim fileUpload = HttpContext.Current.Request.Form("File")
        Dim fileName As String = HttpContext.Current.Request.Form("NamaFile")
        Dim label As String = HttpContext.Current.Request.Form("Label")
        Dim noId As String = HttpContext.Current.Request.Form("No_ID")
        Dim noPendahuluan As String = HttpContext.Current.Request.Form("No_Pendahuluan")
        Dim jnsPendahuluan As String = "PD"
        Dim newId As String = Date.Now.ToString("yyyyMMddHHmmsssssss")
        ...
        Try
            ' Specify the file path where you want to save the uploaded file
            Dim folderPath As String = Server.MapPath($"~/UPLOAD/PENDAHULUAN DAN TUNTUTAN/{jnsPendahuluan}/")
            Dim savePath As String = Path.Combine(folderPath, $"{jnsPendahuluan}{noPendahuluan}{Session("ssusrID")}_{newId}{fileExtension}")
            ...
            If noId.Contains("newId") Then
                'new
                queryRB = New Query()
                Dim query3 As String = $"INSERT INTO SMKB_Pendahuluan_Dok_Upload
                            (No_ID, No_Pendahuluan, Jenis_Pendahuluan, Folder, File_Name) 
                            VALUES (@noId, @nopendahuluan, @jenis, @folder, @filename)"
                param.Clear()
                param.Add(New SqlParameter("@noId", newId))
                param.Add(New SqlParameter("@nopendahuluan", noPendahuluan))
                param.Add(New SqlParameter("@jenis", jnsPendahuluan))
                param.Add(New SqlParameter("@folder", $"{jnsPendahuluan}{noPendahuluan}{Session("ssusrID")}_{newId}{fileExtension}"))
                param.Add(New SqlParameter("@filename", label))

                'Dim db As New DBKewConn
                Dim result As String = RbQueryCmd("No_ID", newId, query3, param)
            ...
            Else
                ...
                param.Clear()
                Dim query2 As String = $"UPDATE SMKB_Pendahuluan_Dok_Upload
                                        Set File_Name = @filename
                                        Where No_Id=@noId"

                param.Add(New SqlParameter("@filename", label))
                param.Add(New SqlParameter("@noId", newId))
                Dim result As String = RbQueryCmd("No_Id", newId, query2, param)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Function PadamFail(postData As String) As String
        Dim resp As New ResponseRepository
        Dim resultDt As New Dictionary(Of String, Object)
        ...
        Dim query As String = $"Select Folder From SMKB_Pendahuluan_Dok_Upload
                                Where No_ID=@noId And No_Pendahuluan =@noPendahuluan"

        param.Add(New SqlParameter("@noId", postDt("TargetId")))
        param.Add(New SqlParameter("@noPendahuluan", postDt("NoPendahuluan")))

        dt = db.Read(query, param)
        Dim Folder As String = ""

        If dt.Rows.Count > 0 Then
            Folder = dt.Rows(0).Item("Folder")
            Dim query2 As String = $"Delete From SMKB_Pendahuluan_Dok_Upload
                            Where No_ID=@noId And No_Pendahuluan =@noPendahuluan"

            param.Clear()
            param.Add(New SqlParameter("@noId", postDt("TargetId")))
            param.Add(New SqlParameter("@noPendahuluan", postDt("NoPendahuluan")))
            Dim result As String = db.Process(query2, param)
            ...
            DeleteFile(Server.MapPath($"~/UPLOAD/PENDAHULUAN DAN TUNTUTAN/PD/{Folder}"))
