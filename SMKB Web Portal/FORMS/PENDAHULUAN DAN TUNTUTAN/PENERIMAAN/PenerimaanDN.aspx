...
<div class="row">
 ...
 <!-- Modal Penerimaan -->
         ...
         <!-- Modal content-->
         <div class="modal-content">
         <div class="modal-header"><h4>Kelulusan Bendahari</h4>
         <button type="button" class="close" data-dismiss="modal"></button>
                  ...
                  <asp:Panel ID="Panel3" runat="server" >
                              <div class="form-row">  
                                 ...
                                 <div class="form-group col-sm-6">
                                     ...
                                     <label class="input-group__label" for="Disahkan Oleh">Kelulusan Oleh</label>                                        
                                      <%--<asp:TextBox ID="txtNamaP" runat="server" Width="100%" class="form-control input-sm" style="background-color:#f3f3f3"></asp:TextBox>--%>
                                 </div>                                    
                                 <div class="form-group col-sm-6">
                                     <input type="text" ID="jwtnPengesah"  Width="100%" class="input-group__input form-control"/>                                       
                                     <label class="input-group__label" for="Jawatan Pengesah">Jawatan </label>                                        
                                 </div>                               
                            </div>

                             <fieldset>
                               <h6>Kelulusan Tuntutan Dalam Negara</h6>

                               <div>
                                 <input type="radio"  class="radioBtnClass" name="status" value="05" checked />
                                 <label for="huey">Lulus</label>
                               </div>

                               <div>
                                 <input type="radio" class="radioBtnClass" name="status" value="14" />
                                 <label for="dewey">Tidak Lulus</label>
                               </div>
 ...
 <div class="modal fade" id="saveConfirmationModalDeleteLampiran" tabindex="-1" role="dialog" aria-labelledby="saveConfirmationModalLabel">
     <div class="modal-dialog " role="document">
         <div class="modal-content">
             <div class="modal-header">
                 <h5 class="modal-title" id="saveConfirmationModalLabelDeleteLampiran">Lulus</h5>
             ...
             <div class="modal-footer">
                 <button type="button" class="btn btn-danger" data-dismiss="modal">Tidak Lulus</button>
...
<script type="text/javascript">
 ...
 function setDataPeribadi(data) {
    ...
    async function CalculateElaun($tr) {
        console.log("masuk CalculateElaun ")
        var $ddlTugas = $tr.find(".JenisTugasElaunMkn-list.selection");
        var $ddlTempat = $tr.find(".JnstempatElaunMkn-list.selection");
    ...
    async function GetAmaunPeratusMkn(jnsTugas, jnsTempat) {
        ...
        try {
            ...
            const response = await fetch('PenerimaanDN_WS_New.asmx/kiraElaunMakan', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    async function simpanTotalPelbagai(id, total) {
        ...
        return new Promise((resolve, reject) => {
            $.ajax({
                url: 'PenerimaanDN_WS_New.asmx/SaveJumlahPelbagai',
                method: 'POST',
    ...
    async function AjaxGetDataSumbangan(id) {
        ...
        try {
            ...
            const response = await fetch('PenerimaanDN_WS_New.asmx/GetDatSumbangan', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    async function SetDataSumbanganRows(totalClone, objOrder) {
        ...
        console.log("masuk SetDataSumbanganRows ")
        if (objOrder !== null && objOrder !== undefined) {   //semak berapa object yang ada
            totalClone = objOrder.length;
        }
        ...
        while (counter <= totalClone) {
            ...
            generateDropdown_list("#" + newId_JnsTabung, "PenerimaanDN_WS_New.asmx/GetJenisSumbangan")
        ...
        $('#tblSumbangan tbody tr').each(function (index, obj) {
            ...
            if (index > 0) {
                ...
                if ($(obj).find('.JenisSumbangan-list > select').val() !== "") {
                    ...
                    item.tabungDN.GroupSumbanganDN.push(listSumbangan);
                }
            }
    ...
    async function saveRecordSumbangan(list) {
        ...
        return new Promise((resolve, reject) => {
            $.ajax({
                url: 'PenerimaanDN_WS_New.asmx/SaveRecordSumbanganDN',
                method: 'POST',
