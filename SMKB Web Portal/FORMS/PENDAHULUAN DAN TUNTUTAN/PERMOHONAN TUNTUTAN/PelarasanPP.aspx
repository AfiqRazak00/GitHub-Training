                    ...
                    <tr>
                        ...
                        <td>
                             <div class="form-group col-md-10">
                              <input type="text" ID="txtPendDiberi" class="input-group__input form-control input-sm" style="text-align:right; font-weight: bold;" value="0.00" readonly />
     ...
     $.ajax({
         ...
         async function SetDataKeperluanKepadaRows(totalClone, objOrder) {
             ...
             while (counter <= totalClone) {
                 ...
                 hargaTextbox.val(parseFloat(obj[counter - 1].Jumlah_anggaran).toFixed(2));
             ...
             $('#totalKt').val(parseFloat(total).toFixed(2))  //total harga
         }
         ...
         async function setValueToRow_HdrPermohonan(orderDetail) {  //ni tuk set value bila klik pada gridview permohonan
             ...
             $('#ddlTahun').val(orderDetail[0].Tahun_Tuntut)
             $('#txtTujuan').val(orderDetail[0].Tujuan_Tuntutan)
             $('#tkhMohon').val(orderDetail[0].Tarikh_Mohon)
             $('#ddlKPtj').val(orderDetail[0].Kod_PTJ)
             $('#ddlKumpWang').val(orderDetail[0].Kod_Kump_Wang)
             $('#ddlKO').val(orderDetail[0].Kod_Operasi)
             $('#ddlKP').val(orderDetail[0].Kod_Projek)
             $('#selectedNoPendahuluan').val(orderDetail[0].No_Pendahuluan)
             $('#selectedJumlah').val(orderDetail[0].Jum_Pendahuluan)       
         ...
         $('#tab-Rumusan').click(async function () {
             ...
             if (jumPilih === "" || jumPilih === undefined) {
                 jumPilih = 0.00
                 console.log(jumPilih)
                 $('#txtPendDiberi').val(0.00);
             }
