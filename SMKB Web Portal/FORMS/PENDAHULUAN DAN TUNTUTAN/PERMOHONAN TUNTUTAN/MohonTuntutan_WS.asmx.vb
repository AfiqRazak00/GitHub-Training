...
Public Class MohonTuntutan_WS
    ...
    Private Function GetDataKumpWang(kod As String) As DataTable
        ...
        Dim param As New List(Of SqlParameter)
        If kod <> "" Then
            query &= "AND A.Status = 1 and (A.Kod_Kump_Wang LIKE '%' + @kod + '%' or B.Butiran LIKE '%' + @kod2 + '%') and A.Kod_Vot =@kod3 ORDER BY A.Kod_Kump_Wang"
            'Else
            '    query &= " where A.Status = 1 and A.Kod_Vot =@kod3 order by A.Kod_Kump_Wang"
        End If

        query &= "ORDER BY A.Kod_Kump_Wang"
    ...
    <System.Web.Services.WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function GetJenisKenderaan(ByVal q As String, ByVal nostaf As String) As String


        Dim tmpDT As DataTable = GetJenisKenderaanPilih(q, nostaf)
        Return JsonConvert.SerializeObject(tmpDT)
    End Function

    Private Function GetJenisKenderaanPilih(kod As String, nostaf As String) As DataTable
        Dim db As New DBKewConn
    ...
    <System.Web.Services.WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function GetDataKenderaan(ByVal q As String, ByVal nostaf As String) As String


        Dim tmpDT As DataTable = GetDataKenderaanPjln(q, nostaf)
        Return JsonConvert.SerializeObject(tmpDT)
    End Function

    Private Function GetDataKenderaanPjln(kod As String, nostaf As String) As DataTable
        Dim db As New DBKewConn
        Dim query As String = "SELECT No_Staf, No_Kenderaan as value,No_Kenderaan as text, Jenis_Kenderaan
        FROM SMKB_Tuntutan_Dftr_Kenderaan WHERE No_Staf = @nostaf"
        Dim param As New List(Of SqlParameter)
    ...
    Private Function GetDataOperasi(kod As String) As DataTable
        ...
        Dim param As New List(Of SqlParameter)
        If kod <> "" Then
            query &= "AND A.Status = 1 AND (A.Kod_Operasi Like '%' + @kod + '%' or B.Butiran LIKE '%' + @kod2 + '%') and A.Kod_Kump_Wang =@kod3 ORDER BY by A.Kod_Operasi"
            'Else
    ...
    Private Function GetKodPtjList(kod As String) As DataTable
        ...
        Kod_PTJ +' - '+B.Pejabat as text
        FROM SMKB_COA_Master A
		INNER JOIN VPejabat B ON LEFT(A.Kod_PTJ,2)=B.KodPejabat
		WHERE A.Status='1'"

        Dim param As New List(Of SqlParameter)
        If kod <> "" Then
            query &= "AND Kod_PTJ LIKE '%' + @kod + '%' OR B.Pejabat LIKE '%' + @kod2 + '%' OR B.Singkatan LIKE '%' + @kod3 + '%'"
    ...
    Private Function GetDataProjek(kod As String) As DataTable
        Dim db As New DBKewConn
        'EDIT BY AIDA(03/04/2024)
        Dim query As String = "SELECT DISTINCT A.Kod_Projek as value,A.Kod_Projek+' - '+B.Butiran as text 
								FROM SMKB_COA_Master A
    ...
    Private Function GetRecord_SenaraiSendiri(category_filter As String, tkhMula As DateTime, tkhTamat As DateTime, staffP As String) As DataTable
        ...
        Dim query As String = "SELECT a.No_Tuntutan, a.Tujuan_Tuntutan,  a.PTj, a.Bulan_Tuntut, a.Tahun_Tuntut, a.Status, a.No_Pendahuluan,
                ...
                WHERE  (b.Kod_Modul = '09') AND (b.Kod_Status_Dok = '02') AND (a.Jenis_Tuntutan ='DN')  AND  a.No_Staf = @staffP " & tarikhQuery & " order by a.Tarikh_Mohon desc"
    ...
    Private Function InsertDataItem(listKenyataan As tblKenyataan)
        ...
        Dim query As String = "INSERT INTO SMKB_Tuntutan_Dlm_Kenyataan (No_Tuntutan, Bil, Tarikh, Masa_Bertolak, 
                            Masa_Sampai, Tujuan, Flag_Mula, Flag_Tamat, No_Kend, Flag_Kend_Sendiri, Jarak,Jenis_Kenderaan)
                            VALUES(@NoTuntutan, @bil,@tkhMula, @MasaMula, @MasaTamat, @Tujuan, @flagMula, @flagTamat, @No_Kend, @FlagKendSend, @Jarak, @Jenis_Kenderaan)"
        Dim param As New List(Of SqlParameter)
        ...
        param.Add(New SqlParameter("@Jenis_Kenderaan", listKenyataan.JnsKenderaan))
    ...
    Private Function UpdateOrderDetail(listKenyataan As tblKenyataan)
        ...
        set Tujuan = @Tujuan, Tarikh = @tkhMula, Masa_Bertolak = @MasaMula, Jarak = @Jarak,Jenis_Kenderaan = @Jenis_Kenderaan 
    ...
    Private Function GetDataKenyataan(id As String) As DataTable
        ...
        Dim query As String = "SELECT a.No_Tuntutan, a.Bil, FORMAT(a.Tarikh, 'yyyy-MM-dd') as Tarikh,a.Jarak,a.Masa_Bertolak,
                                substring(CONVERT(varchar,a.Masa_Bertolak,108),1,2) as jamTolak, substring(CONVERT(varchar,a.Masa_Bertolak,108),4,2) as minitTolak, 
                                a.Masa_Sampai,substring(CONVERT(varchar, a.Masa_Sampai,108),1,2) as jamSampai, substring(CONVERT(varchar, a.Masa_Sampai,108),4,2) as minitSampai,
                                a.Tujuan, a.Flag_Mula, a.Flag_Tamat, a.No_Kend, a.Flag_Kend_Sendiri, a.Flag_Sokong_Kend_Sendiri, a.Jenis_Kenderaan, b.Butiran
                                FROM  SMKB_Tuntutan_Dlm_Kenyataan as a INNER JOIN
                                SMKB_Lookup_Detail as b  ON a.Jenis_Kenderaan = b.Kod_Detail 
                                WHERE    b.Kod='AC09'  AND a.No_Tuntutan = @No_Permohonan   ORDER BY a.Bil ASC"
    ...
    Private Function GetDataForElaunPerjln(id As String) As DataTable
        ...
        Dim query As String = $"SELECT TOP 1 No_Tuntutan, No_Kend, jumjarak, Jenis_Kenderaan, Butiran,KM, No_Staf,
                ...
                SMKB_Tuntutan_Dftr_Kenderaan AS c ON b.No_Kend = c.No_Kenderaan  INNER JOIN
                SMKB_Lookup_Detail AS d ON c.Jenis_Kenderaan = d.Kod_Detail INNER JOIN 
                SMKB_CLM_KdrKenderaan as h ON h.KodKend = c.Jenis_Kenderaan  AND H.KodKend =  D.Kod_Detail
                WHERE        (d.Kod = 'AC09') AND a.No_Tuntutan=@No_Permohonan AND c.Jenis_Kenderaan IN('02','07')
    ...
    <System.Web.Services.WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function SimpanStafKongsi(selectedData As String(), noTuntut As String) As String
        Dim resp As New ResponseRepository
        Dim temps As String
        ...
        For Each data As String In selectedData
            If SemakStafAK(noTuntut, data) = 0 Then
                If SimpanStafAK_(noTuntut, data) = "OK" Then
                    'SendEmail(noTuntut, data)
                    success = +1
    ...
    Private Function SemakStafAK(noTuntutan As String, No_Staf As String) As String
        Dim db As New DBKewConn
        Dim JumRekod As Integer
        Dim query As String = $"SELECT No_Tuntutan,No_Staf FROM SMKB_Tuntutan_KongsiBilik WHERE No_Tuntutan = @No_Tuntutan and No_Staf =@No_Staf"
        Dim param As New List(Of SqlParameter)
        param.Add(New SqlParameter("@No_Tuntutan", noTuntutan))
    ...
    Private Function SimpanStafAK_(noTuntut As String, data As String) As String
        Dim db As New DBKewConn
        If noTuntut = "" Then
            Exit Function
        End If
        ...
        Dim query As String = "INSERT INTO SMKB_Tuntutan_KongsiBilik VALUES(@No_Tuntutan,@No_Staf)"
            Dim param As New List(Of SqlParameter)

        param.Add(New SqlParameter("@No_Tuntutan", noTuntut))
