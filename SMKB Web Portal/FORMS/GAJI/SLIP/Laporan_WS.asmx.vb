	...
	Private Function GetRecord_StafKontrak(tahun As String, syarikat As String, selectedWarga As String) As DataTable
		...
		Try
			Using sqlconn As New SqlConnection(dbSMKB.strCon)
				...
				Dim query As String = "SELECT A.MS01_NoStaf, B.MS01_Nama, (SELECT MS02_GredGajiS FROM [DEVMIS\SQL_INS01].dbStaf.dbo.MS02_Perjawatan WHERE MS01_NoStaf = A.MS01_NoStaf) AS Gred,
									YEAR(CONVERT(DATETIME, B.MS01_TkhLahir, 103)) AS TahunLahir,
									(YEAR(GETDATE()) - YEAR(CONVERT(DATETIME, B.MS01_TkhLahir, 103))) AS Umur,
									B.MS01_Warganegara, (SELECT NamaNegara FROM [DEVMIS\SQL_INS01].dbStaf.dbo.MS_Negara WHERE KodNegara = B.MS01_Warganegara) AS Warganegara,
									A.MS09_TkhMula,CONVERT(VARCHAR(10), A.MS09_TkhMula, 103) AS TkhMulaFormatted, A.MS09_TkhTamat,
									(SELECT MS02_JumlahGajiS FROM [DEVMIS\SQL_INS01].dbStaf.dbo.MS02_Perjawatan WHERE MS01_NoStaf = A.MS01_NoStaf) AS gaji_pokok,
									CASE
									...
									FROM [DEVMIS\SQL_INS01].dbStaf.dbo.MS09_Kontrak A
									INNER JOIN [DEVMIS\SQL_INS01].dbStaf.dbo.MS01_Peribadi B ON B.MS01_NoStaf = A.MS01_NoStaf
