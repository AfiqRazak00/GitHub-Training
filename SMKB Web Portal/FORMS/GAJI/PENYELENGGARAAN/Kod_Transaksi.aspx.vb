...
Public Class Kod_Transaksi
    ...
    Private Function fCreateDtJenis() As DataTable
        ...
        Try
            ...
            If searchDDL = "-" Then
                strSql = "SELECT jenis_trans,kod_Trans,butiran,Kira_Kwsp,Kira_Perkeso,Kira_Cukai,Kira_Pencen,isnull(Vot_Tetap,'-') as vot_tetap,isnull(Vot_Bukan_Tetap,'-') as Vot_Bukan_Tetap,masuk_ap,jenis,kod_kerajaan from smkb_gaji_kod_trans order by kod_trans"

            Else
                strSql = "SELECT jenis_trans,kod_Trans,butiran,Kira_Kwsp,Kira_Perkeso,Kira_Cukai,Kira_Pencen,isnull(Vot_Tetap,'-') as vot_tetap,isnull(Vot_Bukan_Tetap,'-') as Vot_Bukan_Tetap,masuk_ap,jenis,kod_kerajaan from smkb_gaji_kod_trans where jenis_trans = '" & searchDDL & "' order by kod_trans"
            ...
            For i As Integer = 0 To dsKod.Tables(0).Rows.Count - 1
                jenistrans = dsKod.Tables(0).Rows(i)(0).ToString
                kod = dsKod.Tables(0).Rows(i)(1).ToString
                butir = dsKod.Tables(0).Rows(i)(2).ToString
                kwsp = dsKod.Tables(0).Rows(i)(3).ToString
                perkeso = dsKod.Tables(0).Rows(i)(4).ToString
                cukai = dsKod.Tables(0).Rows(i)(5).ToString
                pencen = dsKod.Tables(0).Rows(i)(6).ToString
                vottetap = dsKod.Tables(0).Rows(i)(7).ToString
                votxtetap = dsKod.Tables(0).Rows(i)(8).ToString
                jenis = dsKod.Tables(0).Rows(i)(9).ToString
                agensi = dsKod.Tables(0).Rows(i)(10).ToString



                dt.Rows.Add(jenistrans, kod, butir, kwsp, perkeso, cukai, pencen, vottetap, votxtetap, jenis, agensi)
            Next
    ...
    Private Sub gvJenis_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvJenis.RowCommand
        Try
            ...
            If e.CommandName = "EditRow" Then
                ...
                DropDownList1.SelectedValue = selectedRow.Cells(0).Text.ToString
                txtJenis.Text = selectedRow.Cells(1).Text.ToString
                txtButir.Text = selectedRow.Cells(2).Text.ToString
                ddlVotTetap.SelectedValue = selectedRow.Cells(7).Text.ToString
                ddlVotBknTetap.SelectedValue = selectedRow.Cells(8).Text.ToString
                ...
                Else
                    rbCukai.SelectedValue = "2"
                End If
                If selectedRow.Cells(6).Text.ToString = "True" Then
            ...
            ElseIf e.CommandName = "DeleteRow" Then
                ...
                Dim jenis As String = selectedRow.Cells(1).Text.ToString
    ...
    Protected Sub lbtnSave_Click(sender As Object, e As EventArgs) Handles lbtnSave.ServerClick
        ...
        Dim dbconn As New DBKewConn
        Try
            Dim strJenis As String = Trim(DropDownList1.SelectedValue.TrimEnd)
            ...
            Dim pencen As Byte
            Dim masukap As Byte
            Dim staMohon As Byte

            Dim message As String = "Rekod tidak disimpan. Sila masukkan kod transaksi."

            'If strKod = "" Then
            '    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Pop", "alert('" + message + "'),ShowPopup('2');", True)
            '    Exit Sub
            'End If
            ...
            If rbMohon.SelectedValue = "1" Then
                staMohon = 1
            Else
                staMohon = 0
            End If
            ...
            If isave = "1" Then
                ...
                If strKod = "" Then
                    message = "Sila masukkan Kod Transaksi!"
                ...
                If fCheckRec(strSql) > 0 Then
                    lblModalMessaage.Text = "Rekod tidak disimpan. Kod transaksi ini telah wujud." 'message di modal
                    ClientScript.RegisterStartupScript([GetType](), "alert", "SaveSucces();", True)
                    Exit Sub
                Else
                    strSql = "insert into SMKB_Gaji_kod_Trans (Jenis_Trans,kod_trans,Butiran,Kira_Kwsp,Kira_Perkeso,Kira_Cukai,Kira_Pencen,Vot_Tetap,Vot_Bukan_Tetap,Masuk_AP,Kod_Kerajaan,Jenis,Status_Mohon) values (@JenisTrans,@Kod,@Butir,@kwsp,@socso,@cukai,@pencen,@vottetap,@votxtetap,@masukap,@agensi,@jenis,@stamohon)"
                    Dim paramSql() As SqlParameter = {
                         New SqlParameter("@JenisTrans", strJenis),
                        ...
                        New SqlParameter("@Butir", strButir),
                            ...
                            New SqlParameter("@stamohon", staMohon)
                    }
                ...
                End If
            Else
                strSql = "UPDATE SMKB_Gaji_kod_Trans SET Butiran=@Butir,Kira_Kwsp=@kwsp,Kira_Perkeso=@socso,Kira_Cukai=@cukai,Kira_Pencen=@pencen,Vot_Tetap=@vottetap,Vot_Bukan_Tetap=@votxtetap,Masuk_AP=@masukap,Kod_Kerajaan=@agensi,Jenis=@jenis,Status_Mohon=@stamohon WHERE kod_Trans=@Kod"
