...
<ToolboxItem(False)>
<ScriptService()>
Public Class PENGESAHAN_STORWS
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function loadListPengesahan(idMohon) As String
        Dim resp As New ResponseRepository


        dtbl = getListPengesahan(idMohon)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function getListPengesahan(idMohon) As DataTable
        Dim dbconn As New DBKewConn
        Dim param As New List(Of SqlParameter)
    ...
    Public Function getMaklumatPemohon(idMohon) As DataTable
        ...
        Dim strSql As String = $"SELECT b.MS01_Nama, b.MS01_NoStaf, b.JawGiliran, b.MS01_TelPejabat, b.MS08_Pejabat, b.Pejabat, CONVERT(VARCHAR(10), MAX(a.Tkh_Tindakan), 103) AS Tkh_Pengesahan
                                FROM SMKB_Status_Dok a, VPeribadi b 
                                WHERE a.Kod_Modul = '28' AND a.Kod_Status_Dok = '02' AND a.No_Rujukan = '{idMohon}' AND a.No_Staf = b.MS01_NoStaf 
								GROUP BY a.Tkh_Tindakan, b.MS01_Nama, b.MS01_NoStaf, b.JawGiliran, b.MS01_TelPejabat, b.Pejabat, b.MS08_Pejabat"
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Private Function InsertPengesahanStatusDok(IdMohon, status_dok)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function updateTdkSah(IdMohon, status_dok) As String
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function updateSah(IdMohon, status_dok) As String
        Dim query As String = "UPDATE SMKB_SI_Order_Hdr SET Status_Dok = @status_dok WHERE No_Mohon = @IdMohon"
        Dim param As New List(Of SqlParameter)()
        param.Add(New SqlParameter("@IdMohon", IdMohon))
        param.Add(New SqlParameter("@status_dok", status_dok))
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function tidakSahStatusDok(IdMohon As String) As String
        Dim dbconn As New DBKewConn
        Dim resp As New ResponseRepository
        ...
        'Update status here
        If updateTdkSah(IdMohon, "10") <> "OK" Then
            queryRB.rollback()
            resp.Failed("Gagal menyimpan rekod 2")
            Return JsonConvert.SerializeObject(resp.GetResult())
        End If

        If InsertPengesahanStatusDok(IdMohon, "10") <> "OK" Then
            queryRB.rollback()
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function sahStatusDok(IdMohon As String) As String
        Dim dbconn As New DBKewConn
        Dim resp As New ResponseRepository
        ...
        'Update status here
        If updateSah(IdMohon, "03") <> "OK" Then
            queryRB.rollback()
            resp.Failed("Gagal menyimpan rekod 2")
            Return JsonConvert.SerializeObject(resp.GetResult())
        End If

        If InsertPengesahanStatusDok(IdMohon, "03") <> "OK" Then
            queryRB.rollback()
