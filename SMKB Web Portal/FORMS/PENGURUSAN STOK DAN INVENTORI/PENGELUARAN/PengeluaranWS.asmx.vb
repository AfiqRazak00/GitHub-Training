...
Imports SMKB_Web_Portal.StorMohonWS

' To allow this Web Service to be called from script, using ASP.NET AJAX, uncomment the following line.
<System.Web.Script.Services.ScriptService()>
<System.Web.Services.WebServiceBinding(ConformsTo:=WsiProfiles.BasicProfile1_1)>
<ToolboxItem(False)>
Public Class PengeluaranWS
    ...
    Public Function fetchSenaraiPengeluaran(category_filter As String, isClicked As Boolean, tkhMula As String, tkhTamat As String) As String
        If isClicked = False Then
            Return JsonConvert.SerializeObject(New DataTable)
        End If

        Using dtUserInfo = fGetSenaraiPengeluaran(category_filter, tkhMula, tkhTamat)
    ...
    Public Function fGetSenaraiPengeluaran(category_filter As String, tkhMula As String, tkhTamat As String) As DataTable
        ...
        Dim query As String = $"SELECT 
                                ...
                                WHERE 
                                    a.Kat_Mohon = 'S' AND a.Status_Dok = '04'" & tarikhQuery & "GROUP BY a.No_Mohon,a.Kod_Ptj;"
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function fetchSenaraiPengeluaranUtama(category_filter As String, isClicked As Boolean, tkhMula As String, tkhTamat As String) As String
        If isClicked = False Then
            Return JsonConvert.SerializeObject(New DataTable)
        End If

        Using dtUserInfo = fGetSenaraiPengeluaranUtama(category_filter, tkhMula, tkhTamat)
            If dtUserInfo.Rows.Count > 0 Then
                Return JsonConvert.SerializeObject(dtUserInfo)
            Else
                Dim errorMessage As New Dictionary(Of String, String)
    ...
    Public Function fGetSenaraiPengeluaranUtama(category_filter As String, tkhMula As String, tkhTamat As String) As DataTable
        Dim dbconn As New DBKewConn
        Dim tarikhQuery As String = ""
        Dim param As New List(Of SqlParameter)
        ...
        Dim query As String = $"SELECT 
                                ...
                                INNER JOIN 
                                    SMKB_SI_Order_Dtl b ON a.No_Mohon = b.No_Mohon
                                WHERE 
                                    a.Kat_Mohon = 'I' AND a.Status_Dok = '04'" & tarikhQuery & "GROUP BY a.No_Mohon,a.Kod_Ptj;"
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function fetchPengeluaranDetails(noMohon As String) As String

        Using dtUserInfo = fGetPengeluaranDetails(noMohon)
            If dtUserInfo.Rows.Count > 0 Then
                Return JsonConvert.SerializeObject(dtUserInfo)
            Else
                Dim errorMessage As New Dictionary(Of String, String)
    ...
    Public Function fGetPengeluaranDetails(noMohon As String) As DataTable
        Dim dbconn As New DBKewConn
        ...
        Dim query As String = $"SELECT DISTINCT
                                ...
                                FROM 
                                    SMKB_SI_Order_Dtl a
                                INNER JOIN 
                                    SMKB_SI_Order_Hdr b ON a.No_Mohon = b.No_Mohon AND b.Kat_Mohon = 'S' AND b.Status_Dok = '04'
                                INNER JOIN 
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function fetchPengeluaranDetailsUtama(noMohon As String) As String

        Using dtUserInfo = fGetPengeluaranDetailsUtama(noMohon)
            If dtUserInfo.Rows.Count > 0 Then
                Return JsonConvert.SerializeObject(dtUserInfo)
            Else
                Dim errorMessage As New Dictionary(Of String, String)
    ...
    Public Function fGetPengeluaranDetailsUtama(noMohon As String) As DataTable
        Dim dbconn As New DBKewConn
        ...
        Dim query As String = $"SELECT DISTINCT
                                    ...
                                    (SELECT SUM(c.Baki_Unit)
									 FROM SMKB_SI_Inventori c
									 WHERE c.Kod_Brg = a.Kod_Brg
									 AND b.Kod_Ptj = c.Kod_Ptj) AS Total_Baki_Unit,
                                    d.Butiran AS NamaBarang, 
                                     STUFF((
		                                SELECT DISTINCT ',' + e.Kod_Lokasi
		                                FROM SMKB_SI_Barang_Stor e
		                                WHERE a.Kod_Brg = e.Kod_Brg AND e.Kod_Ptj = b.Kod_Ptj
		                                FOR XML PATH('')), 1, 1, '') AS Kod_Lokasi,
	                                f.Butiran,
	                                CONCAT(g.KodPejabat, '0000') AS KodPejabat,
	                                g.Pejabat
                                FROM 
                                    SMKB_SI_Order_Dtl a
                                INNER JOIN 
                                    SMKB_SI_Order_Hdr b ON a.No_Mohon = b.No_Mohon AND b.Kat_Mohon = 'I' AND b.Status_Dok = '04'
                                INNER JOIN 
                                    SMKB_SI_Inventori c ON a.Kod_Brg = c.Kod_Brg AND b.Kod_Ptj = c.Kod_Ptj
                                INNER JOIN 
                                    SMKB_SI_Barang_Master d ON a.Kod_Brg = d.Kod_Brg
                                INNER JOIN 
                                    SMKB_SI_Barang_Stor e ON a.Kod_Brg = e.Kod_Brg
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function SimpanPengeluaran(NoMohon) As String
    ...
    Private Function UpdateStatusOrder(NoMohon) As String
        ...
        param.Add(New SqlParameter("@Status_Dok", "06"))
        param.Add(New SqlParameter("@No_Mohon", NoMohon))
    ...
    Private Function InsertStatusDok(staffId, noPermohonan) As String
        ...
        param.Add(New SqlParameter("@Kod_Status_Dok", "06"))
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function SimpanPengeluaranUtama(formDataArray As List(Of OrderDtl), NoMohon As String, kodPtj As String) As String
        Dim resp As New ResponseRepository
        Dim db As New DBKewConn
        Dim Query As String
        ...
        'Update Status_Dok ke SMKB_SI_Order_Hdr
        If UpdateStatusOrderUtama($"{NoMohon}") <> "OK" Then
            queryRB.rollback()
            resp.Failed("Gagal menyimpan rekod ‼️")
            Return JsonConvert.SerializeObject(resp.GetResult())
        End If
        ...
        For Each formData In formDataArray
            While formData.KuantitiLulus > 0
                ...
                Dim bakiSemasa As String = GetBakiSemasa(formData.IdBarang, kodPtj)
                ...
                If dtbl.Rows.Count > 0 Then
                    ...
                    Else
                        ...
                        Baki_Unit = 0
                        'Update Data Ke SMKB_SI_Inventori
                        If UpdateStatusInventoriUtama(Harga, formData.IdBarang, idSykt, ID_Inv, kodPtj) <> "OK" Then
                            queryRB.rollback()
                            resp.Failed("Gagal menyimpan rekod ‼️")
                            Return JsonConvert.SerializeObject(resp.GetResult())
                        End If
                    ...
                    'Update Data Ke SMKB_SI_Inventori
                    If UpdateInventoriUtama(Harga, formData.IdBarang, idSykt, Baki_Unit, ID_Inv, kodPtj) <> "OK" Then
                        queryRB.rollback()
                        resp.Failed("Gagal menyimpan rekod ‼️")
                        Return JsonConvert.SerializeObject(resp.GetResult())
                    End If
                    ...
                    If InsertTransaksiUtama(formData.IdBarang, KuantitiTransaksi, bakiSemasa, Harga, NoMohon, kodPtj) <> "OK" Then
                        queryRB.rollback()
                        resp.Failed("Gagal menyimpan rekod ‼️")
                        Return JsonConvert.SerializeObject(resp.GetResult())
                    End If
    ...
    Private Function InsertTransaksiUtama(IdBarang, KuantitiTransaksi, bakiSemasa, Harga, NoMohon, kodPtj) As String
        Dim db As New DBKewConn
    ...
    Private Function UpdateInventoriUtama(Harga, idBarang, idSykt, Baki_Unit, ID_Inv, kodPtj) As String
        Dim db As New DBKewConn
    ...
    Private Function UpdateStatusInventoriUtama(Harga, idBarang, idSykt, ID_Inv, kodPtj) As String
        Dim db As New DBKewConn
    ...
    Private Function UpdateStatusOrderUtama(NoMohon) As String
        Dim db As New DBKewConn
        Dim query As String = "UPDATE SMKB_SI_Order_Hdr
                                SET Status_Dok = @Status_Dok
                                WHERE No_Mohon = @No_Mohon AND Kat_Stor = @Kat_Stor AND Kat_Mohon = @Kat_Mohon AND Status = @Status"
        Dim param As New List(Of SqlParameter)
