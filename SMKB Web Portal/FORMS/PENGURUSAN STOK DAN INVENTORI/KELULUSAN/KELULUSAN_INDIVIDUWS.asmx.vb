...
<ToolboxItem(False)>
<ScriptService()>
Public Class KELULUSAN_INDIVIDUWS
    ...
    Public Function getListKelulusan(idMohon) As DataTable
        ...
        'retrieve kod ptj
        Dim strSql As String = "SELECT DISTINCT CONCAT(b.Kod_Brg, '-', e.Butiran) AS Kod_Brg_Butiran, 
											b.ID_Mohon_Dtl, 
											b.Kuantiti_Mohon, 
											b.Kuantiti_Lulus, 
											b.Tujuan, 
											(SELECT SUM(c.Baki_Unit) FROM SMKB_SI_Inventori c WHERE c.Kod_Ptj = a.Kod_Ptj AND c.Kod_Brg = b.Kod_Brg) AS Total_Baki_Unit,
											f.Takat_Min, 
											f.Takat_Menokok,
											...
											AND e.Kod_Brg = f.Kod_Brg
											AND f.Kod_Ptj = a.Kod_Ptj
											AND c.Kod_Ptj = a.Kod_Ptj
    ...
    'update status_dok from 02 to 02
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    ...
    Public Function updateKelulusanKuantiti(rowDataArray As List(Of OrderDtl), IdMohon As String) As String
        ...
        If InsertPengesahanStatusDok(IdMohon, "04") <> "OK" Then
            queryRB.rollback()
    ...
    Public Function TidakLulus(IdMohon As String) As String
        ...
        If InsertPengesahanStatusDok(IdMohon, "11") <> "OK" Then
            queryRB.rollback()
            resp.Failed("Gagal menyimpan rekod")
            Return JsonConvert.SerializeObject(resp.GetResult())
        End If
