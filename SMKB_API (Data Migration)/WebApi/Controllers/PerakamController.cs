...
{
    [Authorize]
    public class PerakamController : ApiController
    {
        ...
        [Route("api/perakam/{id}/{orderId}/{app_Id}/{mob_Id}/{logincode_Id}")]
        public IEnumerable<string> Get(int id, string orderId, string app_Id, string mob_Id, string logincode_Id)
        {
            ...
            else
            {
                ...
                if (id == 1)
                {
                    ...
                    return SQLPerakam.CheckOpenGateMasuk(user.UserName.ToString(), app_Id);

                }
                if (id == 2)
                {
                    // get detailuser and app setting
                    //return SQLPerakam.CheckOpenGateKeluar(user.UserName.ToString(), app_Id);

                }
                if (id == 3)
                {
                    // get detailuser and app setting
                    //return SQLPerakam.GetInfo(user.UserName.ToString(), app_Id);
